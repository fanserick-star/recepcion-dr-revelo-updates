// Dr. Revelo WhatsApp Cloud Worker v2.6.8 — mensaje institucional
...
// cloudflare/whatsapp_worker_v2_6_responses.js
var encoder = new TextEncoder();
var decoder = new TextDecoder();
var WEEKDAYS = ["domingo", "lunes", "martes", "mi\xE9rcoles", "jueves", "viernes", "s\xE1bado"];
var MONTHS = ["enero", "febrero", "marzo", "abril", "mayo", "junio", "julio", "agosto", "septiembre", "octubre", "noviembre", "diciembre"];
var CLOUD_TEST_PREFIX = "mobile:whatsapp-cloud-test:";
var CLOUD_TEST_TEMPLATES = /* @__PURE__ */ new Set(["cita_agendada", "recordatorio_cita", "recordatorio_hoy"]);
...
var DEFAULT_HEADER_IMAGE_URL = "https://raw.githubusercontent.com/fanserick-star/recepcion-dr-revelo-updates/main/assets/whatsapp/recordatorios_de_citas_header.jpg";
...
function enabled(v2) {
  return String(v2 ?? "0").trim() === "1";
}
function pad(n) {
  return String(n).padStart(2, "0");
}
function normalizePhone(v2) {
  let d2 = String(v2 ?? "").replace(/\D/g, "");
  if (d2.startsWith("00")) d2 = d2.slice(2);
  if (d2.startsWith("0") && d2.length === 10) d2 = "593" + d2.slice(1);
  return d2;
}
function dateParts(iso) {
  const [y, m2, d2] = String(iso).slice(0, 10).split("-").map(Number);
  return { y, m: m2, d: d2 };
}
function timeParts(t) {
  const [h, m2] = String(t).slice(0, 5).split(":").map(Number);
  return { h, m: m2 };
}
function dateLabel(iso) {
  const { y, m: m2, d: d2 } = dateParts(iso);
  const wd = new Date(Date.UTC(y, m2 - 1, d2, 12)).getUTCDay();
  return `${WEEKDAYS[wd]} ${d2} de ${MONTHS[m2 - 1]} de ${y}`;
}
function timeLabel(t) {
  const { h, m: m2 } = timeParts(t);
  return `${h % 12 || 12}:${pad(m2)} ${h >= 12 ? "PM" : "AM"}`;
}
function recordatorioDateTimeLabel(iso, t) {
  const dp = dateParts(iso), tp = timeParts(t);
  const wd = new Date(Date.UTC(dp.y, dp.m - 1, dp.d, 12)).getUTCDay();
  return `${WEEKDAYS[wd]}, ${dp.d} de ${MONTHS[dp.m - 1]} de ${dp.y} a las ${tp.h % 12 || 12}:${pad(tp.m)} ${tp.h >= 12 ? "p. m." : "a. m."}`;
}
async function sha256(s) {
  const b2 = await crypto.subtle.digest("SHA-256", encoder.encode(String(s)));
  return [...new Uint8Array(b2)].map((x2) => x2.toString(16).padStart(2, "0")).join("");
}
...
async function validMetaSignature(rawBody, header, appSecret) {
  if (!appSecret || !header?.startsWith("sha256=")) return false;
  const sig = hexToBytes(header.slice(7));
  if (!sig) return false;
...
function buildTemplatePayload(c, env) {
  ...
  if (c.body_params?.length) components.push({ type: "body", parameters: c.body_params.map((x2) => ({ type: "text", text: String(x2) })) });
...
function acknowledgementText(action, env = {}) {
  ...
  if (a2 === "TEST_CONFIRMAR") return "Prueba recibida: la confirmaci\xF3n se proces\xF3 correctamente. No se modific\xF3 ninguna cita real.";
  if (a2 === "TEST_CANCELAR") return "Prueba recibida: la respuesta de no asistencia se proces\xF3 correctamente. No se modific\xF3 ninguna cita real.";
...
async function withClient(env, fn) {
  const c = new ut(env.DATABASE_URL);
  try {
    await c.connect();
    return await fn(c);
  } finally {
...
var inboundSchemaReady = false;
function normalizeIntentText(value) {
  return String(value || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9ñ]+/g, " ").replace(/\s+/g, " ").trim();
}
function hasAny(textValue, phrases) {
  return phrases.some((p2) => textValue.includes(p2));
}
...
async function ensureInboundSchema(client) {
  ...
  try {
    ...
    inboundSchemaReady = true;
    return true;
  } catch (e) {
    console.error("whatsapp_inbound_schema_failed", e);
    return false;
  }
}
...
Para agendar, reagendar o realizar cualquier otra consulta, por favor comun\xEDquese directamente con el consultorio:
\u{1F4DE} *${doctorPhone}*
...
async function findInboundTarget(client, message, phone) {
  const contextId = String(message?.context?.id || "").trim();
  if (contextId) {
    const r2 = await client.query(`SELECT source_type,source_id,appointment_date::text appointment_date,appointment_time::text appointment_time,patient_name,phone
    ...
    if (r2.rows?.length) {
      const x2 = r2.rows[0];
      return { sourceType: String(x2.source_type || ""), sourceId: Number(x2.source_id || 0), date: String(x2.appointment_date || "").slice(0, 10), time: String(x2.appointment_time || "").slice(0, 5), patientName: String(x2.patient_name || ""), matchMethod: "contexto_mensaje" };
    }
  }
  const normalized = normalizePhone(phone);
  if (!normalized) return null;
...
async function fetchWhatsAppAudio(message, env) {
  ...
  if (!mediaId) return { mediaId: "", mimeType: "", buffer: null };
  ...
  if (!url.startsWith("https://")) throw new Error("media_url_missing");
  const mediaResp = await fetch(url, { headers: auth });
  if (!mediaResp.ok) throw new Error(`media_download_${mediaResp.status}`);
  const declared = Number(mediaResp.headers.get("content-length") || 0);
  if (declared > 8 * 1024 * 1024) throw new Error("audio_too_large");
  const buffer = await mediaResp.arrayBuffer();
  if (buffer.byteLength > 8 * 1024 * 1024) throw new Error("audio_too_large");
...
async function transcribeWhatsAppAudio(message, env) {
  ...
  try {
    const out = await env.AI.run("@cf/openai/whisper-large-v3-turbo", { audio: arrayBufferToBase64(media.buffer), task: "transcribe", language: "es", vad_filter: true, condition_on_previous_text: false, initial_prompt: "Cita m\xE9dica del consultorio del Dr. Armando Revelo. El paciente responde si asistir\xE1, no asistir\xE1 o desea cambiar su cita." });
    return { ...media, text: String(out?.text || "").trim(), error: "" };
  } catch (e) {
    console.error("whatsapp_audio_transcription_failed", e);
    return { ...media, text: "", error: String(e?.message || e).slice(0, 160) };
  }
}
async function saveInboundRow(client, data) {
  ...
  const r = await client.query(`INSERT INTO whatsapp_cloud.inbound_responses(
    ...
    data.interpretation || "REVISAR",
    Number(data.confidence || 0),
    data.target?.sourceType || null,
    data.target?.sourceId || null,
    data.target?.date || null,
    data.target?.time || null,
    data.target?.patientName || null,
    data.target?.matchMethod || null,
    data.applyResult || null,
    data.resolved ? (/* @__PURE__ */ new Date()).toISOString() : null,
    data.resolved ? "worker" : null,
...
async function handleFreeformInbound(env, message) {
  const messageId = String(message?.id || "").trim(), phone = normalizePhone(message?.from || "");
  ...
  if (type === "audio") {
    try {
      const a2 = await transcribeWhatsAppAudio(message, env);
      transcription = a2.text || "";
      mediaId = a2.mediaId || String(message?.audio?.id || "");
      mediaMimeType = a2.mimeType || String(message?.audio?.mime_type || "");
      audioError = a2.error || "";
    } catch (e) {
      mediaId = String(message?.audio?.id || "");
      mediaMimeType = String(message?.audio?.mime_type || "");
      audioError = String(e?.message || e).slice(0, 160);
      console.error("whatsapp_audio_fetch_failed", e);
    }
  }
  ...
  if (ackAction) {
    ...
    if (ack) {
      try {
        ...
        console.error("whatsapp_freeform_ack_failed", e);
      }
    }
  }
}
async function logButtonInbound(env, message, p2, result) {
  ...
  try {
    await withClient(env, async (client) => {
      ...
      if (!target) target = { sourceType: p2.sourceType, sourceId: p2.sourceId, date: p2.date, time: p2.time, patientName: "", matchMethod: "boton_payload" };
      const confirmed = p2.action === "CONFIRMAR", applied = responseWasApplied(result);
      await saveInboundRow(client, { messageId: String(message?.id || `button:${Date.now()}`), phone: normalizePhone(message?.from || ""), messageType: "button", rawText: confirmed ? "S\xED" : "No", transcription: "", mediaId: "", mediaMimeType: "", interpretation: applied ? confirmed ? "CONFIRMADO" : "NO_ASISTIRA" : "REVISAR", confidence: 100, target, applyResult: result, resolved: applied, resolution: applied ? p2.action : "", rawPayload: { message } });
    });
  } catch (e) {
    console.error("whatsapp_button_log_failed", e);
  }
}
...
function materializeCandidate(r, env) {
  ...
  const d2 = String(r.appointment_date).slice(0, 10), t = String(r.appointment_time).slice(0, 5);
...
async function claim(client, c) {
  const d2 = String(c.appointment_date).slice(0, 10), t = String(c.appointment_time).slice(0, 5);
  const key = await sha256(`${c.source_type}|${c.source_id}|${c.template_name}|${d2}|${t}`);
  c.event_key = key;
...
RETURNING event_key`, [key, c.source_type, c.source_id, c.template_name, d2, t, c.due_at, c.phone, c.patient_name]);
  return !!r.rows?.length;
}
async function markSent(client, c, data) {
  const mid = String(data?.messages?.[0]?.id || "");
  await client.query(`UPDATE whatsapp_cloud.events SET status='SENT',message_id=$2,sent_at=now(),updated_at=now() WHERE event_key=$1`, [c.event_key, mid]);
}
async function markError(client, c, e) {
  await client.query(`UPDATE whatsapp_cloud.events SET status='ERROR',error_code=$2,error_text=$3,updated_at=now() WHERE event_key=$1`, [c.event_key, String(e.code || ""), String(e.message || e).slice(0, 1500)]);
}
async function runScheduler(env) {
  ...
  return withClient(env, async (client) => {
    const rows = await dueCandidates(client, env);
    let claimed = 0, sent = 0, errors = 0, skipped = 0;
    for (const r of rows) {
      ...
      if (!await claim(client, c)) {
        skipped++;
        continue;
      }
      claimed++;
      try {
        const data = await sendMeta(c, env);
        await markSent(client, c, data);
        sent++;
      } catch (e) {
        await markError(client, c, e);
        errors++;
      }
    }
    return { ok: true, candidates: rows.length, claimed, sent, errors, skipped };
  });
}
function extractPayload(message) {
  if (message?.type === "button") return String(message.button?.payload || "");
  if (message?.type === "interactive" && message.interactive?.type === "button_reply") return String(message.interactive.button_reply?.id || "");
  return "";
}
function parseActionPayload(payload) {
  const p2 = String(payload || "").split("|");
  if (p2.length < 5) return null;
  const action = String(p2[0]).toUpperCase(), sourceType = String(p2[1]).toLowerCase(), sourceId = Number.parseInt(p2[2], 10), date = String(p2[3]), time = String(p2[4]).slice(0, 5);
  if (!["CONFIRMAR", "CANCELAR", "TEST_CONFIRMAR", "TEST_CANCELAR"].includes(action) || !["appointment", "staged", "linked"].includes(sourceType) || !Number.isInteger(sourceId) || sourceId <= 0 || !/^\d{4}-\d{2}-\d{2}$/.test(date) || !/^\d{2}:\d{2}$/.test(time)) return null;
  return { action, sourceType: sourceType === "linked" ? "appointment" : sourceType, sourceId, date, time };
}
async function currentSlot(client, p2) {
  const q = p2.sourceType === "staged" ? `SELECT fecha::text d,hora::text t,source_hash::text source_hash FROM public.confirmafy_agenda_items WHERE id=$1` : `SELECT fecha::text d,hora::text t,NULL::text source_hash FROM public.appointments WHERE id=$1`;
  const r = await client.query(q, [p2.sourceId]);
  if (!r.rows?.length) return null;
  const sourceHash = String(r.rows[0].source_hash || "");
  return { date: String(r.rows[0].d).slice(0, 10), time: String(r.rows[0].t).slice(0, 5), isTest: sourceHash.startsWith(CLOUD_TEST_PREFIX) };
}
async function applyResponse(env, p2, messageId, phone) {
  return withClient(env, async (client) => {
    ...
    if (slot.isTest) {
      if (["CONFIRMAR", "TEST_CONFIRMAR"].includes(p2.action)) return "TEST_CONFIRMED";
      if (["CANCELAR", "TEST_CANCELAR"].includes(p2.action)) return "TEST_CANCELLED";
      return "IGNORED_TEST_ACTION";
    }
    if (p2.action.startsWith("TEST_")) return "IGNORED_TEST_ACTION";
    const r = await client.query(`SELECT public.whatsapp_apply_response($1,$2,$3,$4,$5) AS result`, [p2.action, p2.sourceType, p2.sourceId, String(messageId || ""), String(phone || "")]);
    return String(r.rows?.[0]?.result || "UNKNOWN");
  });
}
async function updateStatuses(env, statuses) {
  ...
  await withClient(env, async (client) => {
    for (const s of statuses) {
      ...
      if (st === "delivered") await client.query(`UPDATE whatsapp_cloud.events SET status='DELIVERED',delivered_at=COALESCE(delivered_at,now()),updated_at=now() WHERE message_id=$1`, [mid]);
      else if (st === "read") await client.query(`UPDATE whatsapp_cloud.events SET status='READ',read_at=COALESCE(read_at,now()),delivered_at=COALESCE(delivered_at,now()),updated_at=now() WHERE message_id=$1`, [mid]);
      else if (st === "failed") await client.query(`UPDATE whatsapp_cloud.events SET status='FAILED',error_code=$2,error_text=$3,updated_at=now() WHERE message_id=$1`, [mid, String(err.code || ""), String(err.title || err.message || "Meta report\xF3 fallo").slice(0, 1500)]);
      else if (st === "sent") await client.query(`UPDATE whatsapp_cloud.events SET status=CASE WHEN status='SENDING' THEN 'SENT' ELSE status END,sent_at=COALESCE(sent_at,now()),updated_at=now() WHERE message_id=$1`, [mid]);
    }
  });
}
...
async function receiveWebhook(request, env) {
  ...
  for (const e of body?.entry || []) for (const ch of e?.changes || []) {
    for (const m2 of ch?.value?.messages || []) messages.push(m2);
    for (const st of ch?.value?.statuses || []) statuses.push(st);
  }
  if (statuses.length) await updateStatuses(env, statuses);
  for (const m2 of messages) {
    const p2 = parseActionPayload(extractPayload(m2));
    if (p2) {
      const messageId = String(m2.id || ""), phone = String(m2.from || "");
      let result = "UNKNOWN";
      try {
        result = await applyResponse(env, p2, messageId, phone);
      } catch (e) {
        console.error("whatsapp_apply_response_failed", e);
        continue;
      }
    ...
    if (m2?.type === "text" || m2?.type === "audio") {
      try {
        await handleFreeformInbound(env, m2);
      } catch (e) {
        console.error("whatsapp_freeform_failed", e);
      }
    }
  }
...
var whatsapp_worker_v2_6_responses_default = {
  async fetch(request, env) {
    ...
    if (u.pathname === "/header.jpg") {
      ...
      const r = await fetch(source, { headers: { "User-Agent": "Dr-Revelo-WhatsApp-Worker/2.6.8" } });
    ...
    if (!env.DATABASE_URL || !env.VERIFY_TOKEN || !env.META_APP_SECRET) return text("Webhook not configured", 503);
...
export {
  ...
  classifyInboundText,
  whatsapp_worker_v2_6_responses_default as default,
  extractPayload,
