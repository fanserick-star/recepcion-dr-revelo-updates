...
const DEFAULT_HEADER_IMAGE_URL = "https://dr-revelo-whatsapp-media.drrevelo.workers.dev/header.jpg?v=2";
...
async function sendMeta(c,env){
  ...
  if(!r.ok){ const err=data?.error||{}; const e=new Error(err.message||raw||`HTTP ${r.status}`); e.code=String(err.code||r.status); throw e; }
  return data;
}

function acknowledgementText(action){
  if(String(action||"").toUpperCase()==="CONFIRMAR") return "✅ ¡Gracias por confirmar! Su cita con el Dr. Armando Revelo ha quedado confirmada. Lo esperamos en la fecha y hora indicadas. 😊";
  if(String(action||"").toUpperCase()==="CANCELAR") return "✅ Gracias por avisarnos. Hemos registrado que no podrá asistir a su cita. Si desea reagendar, puede escribirnos por este mismo medio y con gusto le ayudaremos.";
  return "";
}
...
WITH base AS (
  ...
  UNION ALL
  -- Prueba controlada: solo filas marcadas explícitamente como whatsapp-cloud-test se envían de inmediato.
  -- El event_key normal evita que vuelvan a enviarse luego a las 08:00.
  SELECT b.*, 'recordatorio_cita'::text,
         now()
  FROM base b
  WHERE $1::boolean AND b.source_type='staged' AND b.source_hash LIKE 'mobile:whatsapp-cloud-test:%'
  UNION ALL
  ...
  FROM base b WHERE $3::boolean
)
SELECT source_type,source_id,patient_name,phone,fecha::text appointment_date,hora::text appointment_time,kind,due_at
FROM ev
WHERE ((fecha + hora::time) AT TIME ZONE 'America/Guayaquil') > now()
  AND due_at <= now()
  AND due_at > now() - CASE WHEN kind='cita_agendada' THEN interval '1 hour' ELSE interval '4 hours' END
ORDER BY due_at
LIMIT 50`;
...
function materializeCandidate(r,env){
  ...
  if(r.kind==="recordatorio_cita") return {...r,phone,template_name:env.TEMPLATE_RECORDATORIO_CITA||"recordatorio_cita",language:env.LANG_RECORDATORIO_CITA||"es_ES",body_params:[name,recordatorioDateTimeLabel(d,t)],buttons:[`CONFIRMAR|${r.source_type}|${r.source_id}|${d}|${t}`,`CANCELAR|${r.source_type}|${r.source_id}|${d}|${t}`],header_image_id: enabled(env.ENABLE_RECORDATORIO_CITA_LOGO)?String(env.RECORDATORIO_CITA_HEADER_IMAGE_ID||""):""};
  if(r.kind==="recordatorio_hoy") return {...r,phone,template_name:env.TEMPLATE_RECORDATORIO_HOY||"recordatorio_hoy",language:env.LANG_RECORDATORIO_HOY||"es_EC",body_params:[name,timeLabel(t)],buttons:[],header_image_id:String(env.WHATSAPP_HEADER_IMAGE_ID||"")};
  return {...r,phone,template_name:env.TEMPLATE_CITA_AGENDADA||"cita_agendada",language:env.LANG_CITA_AGENDADA||"es_EC",body_params:[name,dateLabel(d),timeLabel(t)],buttons:[],header_image_url:String(env.WHATSAPP_HEADER_IMAGE_URL||DEFAULT_HEADER_IMAGE_URL).trim()};
}
