...
function buildTemplatePayload(c, env){
  const components=[];
  if(c.header_image_id) components.push({type:"header",parameters:[{type:"image",image:{id:c.header_image_id}}]});
...
function materializeCandidate(r,env){
  ...
  return {...r,phone,template_name:env.TEMPLATE_CITA_AGENDADA||"cita_agendada",language:env.LANG_CITA_AGENDADA||"es_EC",body_params:[name,dateLabel(d),timeLabel(t)],buttons:[],header_image_id:String(env.WHATSAPP_HEADER_IMAGE_ID||"")};
}
