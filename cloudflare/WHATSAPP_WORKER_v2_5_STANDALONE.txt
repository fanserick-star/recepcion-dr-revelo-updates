...
async function receiveWebhook(request, env) {
  ...
  for (const m2 of messages) {
    ...
    if (!p2) continue;
    const messageId = String(m2.id || "");
    const phone = String(m2.from || "");
    let result = "UNKNOWN";
    try {
...
var whatsapp_worker_v2_5_reglas_y_pruebas_default = {
