...
import validate_v4451 as validation
...
def fetch(url: str,attempts: int=4,timeout: float=25.0)->bytes:
    ...
    for i in range(max(1,attempts)):
        try:
            sep='&' if '?' in url else '?'
            req=urllib.request.Request(url+sep+'rp='+str(time.time_ns()),headers={'Cache-Control':'no-cache','Pragma':'no-cache','User-Agent':'v4451-safe-release'})
...
def raw_legacy_acceptance(candidate: dict)->None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        require(legacy._local_package_version(install)==VERSION,'Updater 4.4.43 no dejó manifest 4.4.51')
        require(legacy._installed_app_version(install)==VERSION,'Updater 4.4.43 no dejó app 4.4.51')
    ...
    print('RAW_LEGACY_443_ACCEPTANCE_V4451_OK')

def main()->None:
    subprocess.run([sys.executable,str(HERE/'validate_v4451.py')],cwd=ROOT,check=True)
    ...
    git('add','updates/v4_4_51_payment_source_of_truth')
    staged=subprocess.check_output(['git','diff','--cached','--name-only'],cwd=ROOT,text=True).strip()
    if staged:
        git('commit','-m','release: payload v4.4.51 forma de pago desde atención')
    ...
    for item in candidate['files']:
        ...
        if item['path']=='app.py':
            ...
            require('APP_VERSION = "4.4.51"' in text,'Raw app incorrecta')
            require('/api/visits/batch-payment' in text,'Raw perdió endpoint de pago en atención')
            require('window.__v4451PaymentSourceOfTruth' in text,'Raw perdió UI de pago en atención')
            require("const selected=paymentMap.get(key(id.patient_id,id.fecha))||'';" in text,'Raw conserva falso default visual')
            require('method = _payment_from_visit(visit) or "EFECTIVO"' not in text,'Raw conserva fallback SRI inseguro')
