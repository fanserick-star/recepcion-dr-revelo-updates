...
def build()->None:
    subprocess.run([sys.executable,str(HERE/"build_v4451.py")],cwd=ROOT,check=True)
...
def contracts()->None:
    ...
    for marker in (
        ...
        'Selecciona la forma de pago antes de guardar la atención.',
        'SRI 01',
        'SRI 20',
        "const selected=paymentMap.get(key(id.patient_id,id.fecha))||'';",
        'return !paymentMap.get(key(id.patient_id,id.fecha));',
        'La forma de pago no está registrada. Vuelve a la atención',
        ...
        require(marker in app,f"Falta contrato v4.4.51: {marker}")

    require("const selected=paymentMap.get(key(id.patient_id,id.fecha))||'EFECTIVO';" not in app,"Sigue el falso Efectivo visual")
    require('method = _payment_from_visit(visit) or "EFECTIVO"' not in app,"SRI todavía asume Efectivo")
    ...
    start=app.index('V4451_PAYMENT_ATTENTION_JS = r"""')+len('V4451_PAYMENT_ATTENTION_JS = r"""')
    end=app.index('\n"""\n\n    V4451_PAYMENT_ATTENTION_CSS',start)
    js=app[start:end]
    with tempfile.TemporaryDirectory() as td:
        p=pathlib.Path(td)/"v4451.js";p.write_text(js,encoding="utf-8",newline="")
        r=subprocess.run(["node","--check",str(p)],text=True,capture_output=True)
        require(r.returncode==0,f"JS v4.4.51 inválido: {r.stderr}")

    print("V4451_CONTRACT_OK")
...
def legacy_acceptance()->None:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            require(result.get("ok") and result.get("updated"),f"Updater 4.4.43 rechazó v4.4.51: {result}")
    ...
    print("V4451_ACCEPTED_BY_LEGACY_443")
...
def launcher_selftests()->None:
    ...
    print("V4451_LAUNCHER_SELFTESTS_OK")


def main()->None:
    build();contracts();legacy_acceptance();launcher_selftests();print("VALIDATE_V4451_OK")
