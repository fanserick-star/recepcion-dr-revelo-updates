...
import validate_v4452 as validation
    ...
    for i in range(max(1, attempts)):
        try:
            ...
            req = urllib.request.Request(
                ...
                headers={'Cache-Control': 'no-cache', 'Pragma': 'no-cache', 'User-Agent': 'v4452-safe-release'},
            )
...
def raw_legacy_acceptance(candidate: dict) -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        require(legacy._local_package_version(install) == VERSION, 'Updater 4.4.43 no dejó manifest 4.4.52')
        require(legacy._installed_app_version(install) == VERSION, 'Updater 4.4.43 no dejó app 4.4.52')
        ...
        for path, data in sentinels.items():
            require(path.read_bytes() == data, f"Updater tocó protegido: {path.name}")
        require('app_base_4428.py' in (result.get('paths') or []), 'Raw no incluyó app_base_4428.py')
    print('RAW_LEGACY_443_ACCEPTANCE_V4452_OK')


def main() -> None:
    subprocess.run([sys.executable, str(HERE / 'validate_v4452.py')], cwd=ROOT, check=True)
    ...
    current = json.loads(fetch(
        'https://raw.githubusercontent.com/fanserick-star/recepcion-dr-revelo-updates/main/latest-v3.json'
    ).decode('utf-8-sig'))
    ...
    git('add', 'updates/v4_4_52_quick_unlinked_appointment')
    staged = subprocess.check_output(['git', 'diff', '--cached', '--name-only'], cwd=ROOT, text=True).strip()
    if staged:
        git('commit', '-m', 'release: payload v4.4.52 cita rápida sin ficha')
        git('pull', '--rebase', 'origin', 'main')
        git('push', 'origin', 'HEAD:main')

    # Verificar los bytes que realmente descargará la PC antes de mover el canal.
    for item in candidate['files']:
        data = wait_payload(item)
        if item['path'] == 'app.py':
            text = data.decode('utf-8-sig')
            require('APP_VERSION = "4.4.52"' in text, 'Raw app incorrecta')
            require('/api/agenda/unlinked/guarded' in text, 'Raw perdió endpoint cita rápida')
            require('＋ Crear cita nueva' in text, 'Raw perdió botón Crear cita nueva')
            require('core.ConfirmafyAgendaItem(' in text, 'Raw perdió cita staged')
            require('crear_cita_rapida_sin_ficha' in text, 'Raw perdió auditoría cita rápida')
