...
def fetch(url: str,attempts: int=4,timeout: float=25.0)->bytes:
    ...
    for i in range(max(1,attempts)):
        try:
            sep="&" if "?" in url else "?"
            req=urllib.request.Request(url+sep+"rp="+str(time.time_ns()),headers={"Cache-Control":"no-cache","Pragma":"no-cache","User-Agent":"v4449-safe-release"})
            with urllib.request.urlopen(req,timeout=timeout) as r:
                require(getattr(r,"status",200)==200,f"HTTP {getattr(r,'status','?')}");return r.read()
...
def raw_legacy_acceptance(candidate: dict)->None:
    with tempfile.TemporaryDirectory() as td:
        ...
        for path,data in sentinels.items():require(path.read_bytes()==data,f"Updater tocó archivo protegido: {path.name}")
        require("app_base_4428.py" in (result.get("paths") or []),"Actualización Raw no incluyó app_base_4428.py")
    print("RAW_LEGACY_443_ACCEPTANCE_V4449_OK")

def main()->None:
    subprocess.run([sys.executable,str(HERE/"validate_v4449.py")],cwd=ROOT,check=True)
    ...
    # Publicar payload primero; el canal aún permanece en la versión anterior.
    ...
    if staged:
        git("commit","-m","release: payload v4.4.49 agenda rápida y ficha existente")
        git("pull","--rebase","origin","main");git("push","origin","HEAD:main")

    # Todos los bytes que recibirá la PC deben existir en Raw y coincidir por SHA.
    for item in candidate["files"]:
        data=wait_payload(item)
        if item["path"]=="app.py":
            text=data.decode("utf-8-sig")
            require('APP_VERSION = "4.4.49"' in text,"Raw app incorrecta")
            require("Actualizar datos y atender" in text,"Raw app perdió flujo de ficha existente")
        if item["path"]=="app_base_4428.py":
            require(sha(data)==str(item["sha256"]),"Raw app_base incorrecta")

    # Barrera anti-bucle: el updater REAL 4.4.43 debe poder instalar los bytes Raw.
