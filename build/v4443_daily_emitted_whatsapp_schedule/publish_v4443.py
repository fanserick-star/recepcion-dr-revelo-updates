...
def fetch(url:str,attempts:int=12)->bytes:
    ...
    for i in range(attempts):
        try:
            sep='&' if '?' in url else '?'
            req=urllib.request.Request(url+sep+'ts='+str(time.time_ns()),headers={'Cache-Control':'no-cache','User-Agent':'v4443-publisher'})
            with urllib.request.urlopen(req,timeout=20) as r:
                if getattr(r,'status',200)!=200: raise RuntimeError('HTTP '+str(getattr(r,'status','?')))
                return r.read()
        except Exception as exc:
            last=exc;time.sleep(min(5,0.8+i*0.6))
    raise RuntimeError('Raw no propagó: '+url+': '+str(last))


def main():
    subprocess.run([sys.executable,str(ROOT/'build'/'v4443_daily_emitted_whatsapp_schedule'/'validate_v4443.py')],cwd=ROOT,check=True)
    candidate=json.loads((OUT/'candidate_latest.json').read_text(encoding='utf-8'))
    require(candidate.get('version')=='4.4.43' and candidate.get('app_version')=='4.4.43','Candidato incorrecto')

    git('config','user.name','github-actions[bot]')
    git('config','user.email','41898282+github-actions[bot]@users.noreply.github.com')
    git('add','updates/v4_4_43_daily_emitted_whatsapp_schedule')
    staged=subprocess.check_output(['git','diff','--cached','--name-only'],cwd=ROOT,text=True).strip()
    if staged:
        git('commit','-m','release: payload v4.4.43 emitidas y horario WhatsApp')
    ...
    git('commit','-m','release: v4.4.43 emitidas hoy y horario WhatsApp')
    git('push','origin','HEAD:main')
    ...
    require(remote.get('version')=='4.4.43' and remote.get('app_version')=='4.4.43','Canal no propagó 4.4.43')
