...
VERSION='4.3.69'
BASE_VERSION='4.3.68'
BASE_SHA='4d8ac7054c1697c6a98a6889c7211ece3fe8c0d67d7594ff8c72473bd3f16dd3'
oldroot=ROOT/'updates'/'v468'
parts=sorted(oldroot.glob('app.part*'),key=lambda p:int(p.name.split('part')[-1]))
raw=b''.join(p.read_bytes() for p in parts)
if hashlib.sha256(raw).hexdigest()!=BASE_SHA:
    raise SystemExit('La base v4.3.68 no coincide con la publicada')
s=raw.decode('utf-8')
...
one('APP_VERSION = "4.3.68"','APP_VERSION = "4.3.69"','APP_VERSION')
one('/v460/overlay.css?v=4.3.68','/v460/overlay.css?v=4.3.69','overlay css cache')
one('/v460/overlay.js?v=4.3.68','/v460/overlay.js?v=4.3.69','overlay js cache')

def patch_overlay(js):
    old="const VERSION='4.3.68';"
    if js.count(old)!=1: raise SystemExit('Versión overlay base no encontrada')
    js=js.replace(old,"const VERSION='4.3.69';",1)

    oldcss='.v467-agenda-modal-shell{width:min(600px,calc(100vw - 56px))!important;max-width:600px!important;padding:16px 18px!important;border-radius:17px!important}'
    newcss='.v467-agenda-modal-shell{width:min(600px,calc(100vw - 56px))!important;max-width:600px!important;height:auto!important;min-height:0!important;max-height:calc(100vh - 44px)!important;padding:16px 18px!important;border-radius:17px!important;overflow:auto!important}'
    if js.count(oldcss)!=1: raise SystemExit(f'CSS agenda shell: hubo {js.count(oldcss)} coincidencias')
    js=js.replace(oldcss,newcss,1)

    fix=r'''
;(()=>{
  function clearAgendaModalShell(){
    document.querySelectorAll('.v467-agenda-modal-shell').forEach(el=>el.classList.remove('v467-agenda-modal-shell'));
  }
...
'''
    if '__v469ModalGuard' in js: raise SystemExit('Guard v4.3.69 ya existe')
    return js+fix
rewrite_js_assignment('V460_OVERLAY_JS',patch_overlay)

out=ROOT/'updates'/'v469';out.mkdir(parents=True,exist_ok=True)
...
print('V469_BUILT',len(raw),meta['app_sha256'],'parts',len(chunks))
