...
VERSION='4.3.60'
BASE_VERSION='4.3.59'
BASE_SHA='edb50022e37bc08e0ce2cfc23cc2e0c9d4be32fe7ea0353b4f47860549361321'
oldroot=ROOT/'updates'/'v459'
parts=sorted(oldroot.glob('app.part*'),key=lambda p:int(p.name.split('part')[-1]))
raw=b''.join(p.read_bytes() for p in parts)
if hashlib.sha256(raw).hexdigest()!=BASE_SHA:
    raise SystemExit('La base v4.3.59 no coincide con la publicada')
...
if old not in s:
    raise SystemExit('No se encontró la inyección UI v4.3.59')
...
for i,ch in enumerate(chunks,1):
    (out/f'app.part{i}').write_text(ch,encoding='utf-8',newline='')
rebuilt=b''.join((out/f'app.part{i}').read_bytes() for i in range(1,len(chunks)+1))
if rebuilt!=final:
    raise SystemExit('Las partes no reconstruyen el app.py exacto')
