...
def fetch(url: str, attempts: int = 4, timeout: float = 25.0) -> bytes:
    ...
    for i in range(attempts):
        try:
            sep='&' if '?' in url else '?'
            req=urllib.request.Request(url+sep+'rp='+str(time.time_ns()),headers={'Cache-Control':'no-cache','Pragma':'no-cache','User-Agent':'v4453-safe-release'})
...
def raw_legacy_acceptance(candidate: dict) -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        require(legacy._local_package_version(install)==VERSION,'Updater 4.4.43 no dejó manifest 4.4.53')
        require(legacy._installed_app_version(install)==VERSION,'Updater 4.4.43 no dejó app 4.4.53')
    ...
    print('RAW_LEGACY_443_ACCEPTANCE_V4453_OK')


def main() -> None:
    subprocess.run([sys.executable,str(HERE/'build_v4453.py')],cwd=ROOT,check=True)
    app_text=(OUT/'app.py').read_text(encoding='utf-8-sig')
    require('APP_VERSION = "4.4.53"' in app_text,'Versión app incorrecta')
    require("document.querySelector('#agendaDate')||box?.querySelector('input[type=\\\"date\\\"]')" in app_text,'No usa agendaDate global')
    require("document.querySelector('#agendaTime')||box?.querySelector('input[type=\\\"time\\\"]')" in app_text,'No usa agendaTime global')
    require('/api/agenda/unlinked/guarded' in app_text,'Perdió cita rápida')
    require('core.ConfirmafyAgendaItem(' in app_text,'Perdió staged')
    compile(app_text,'app.py','exec')
    print('V4453_CONTRACT_OK')
    ...
    git('add','updates/v4_4_53_slot_capture_fix')
    staged=subprocess.check_output(['git','diff','--cached','--name-only'],cwd=ROOT,text=True).strip()
    if staged:
        git('commit','-m','release: payload v4.4.53 captura de horario')
    ...
    for item in candidate['files']:
        ...
        if item['path']=='app.py':
            ...
            require('APP_VERSION = "4.4.53"' in text,'Raw app incorrecta')
            require("document.querySelector('#agendaDate')" in text,'Raw perdió agendaDate global')
            require("document.querySelector('#agendaTime')" in text,'Raw perdió agendaTime global')
    ...
    staged=subprocess.check_output(['git','diff','--cached','--name-only'],cwd=ROOT,text=True).strip()
    if staged:
        git('commit','-m','release: publicar v4.4.53 horario de cita rápida')
        git('pull','--rebase','origin','main'); git('push','origin','HEAD:main')
    print('PUBLISH_V4453_OK')
