...
def expected_server_version() -> str:
    try:
        manifest = json.loads((ROOT / "update_manifest.json").read_text(encoding="utf-8"))
        value = str(manifest.get("runtime_version") or manifest.get("app_version") or manifest.get("version") or "").strip()
        if value:
            return value
    except Exception:
        pass
...
def stop_server_on_port() -> None:
    ...
    for _ in range(2):
        ...
        try:
            ...
            try:
                subprocess.run(
                    ["powershell", "-NoProfile", "-NonInteractive", "-WindowStyle", "Hidden", "-Command", f"Stop-Process -Id {pid} -Force -ErrorAction SilentlyContinue"],
                    stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                    timeout=5, check=False,
                    creationflags=getattr(subprocess, "CREATE_NO_WINDOW", 0),
                )
...
def start_server() -> None:
    ...
    entry = (ROOT / "APP_ENTRY.py") if (ROOT / "APP_ENTRY.py").exists() else (ROOT / "app.py")
    subprocess.Popen(
