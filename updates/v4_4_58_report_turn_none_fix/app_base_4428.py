...
def build_report_data(rows):
    ...
    service_totals = {}
    detail_rows = []
    for (fecha, pid), items in sorted(patient_days.items(), key=lambda kv: (kv[0][0], turns[kv[0]] is None, turns[kv[0]] or 0, min(v.id for v, _p in kv[1]))):
        p = items[0][1]
...
V478_REMIX_JS = r""";(()=>{
  ...
  function v4457ConsultationTurnMap(groups){
