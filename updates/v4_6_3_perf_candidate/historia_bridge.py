...
_PRESENCE_CACHE: dict | None = None
_PRESENCE_CACHE_MONOTONIC = 0.0
_PRESENCE_CACHE_SECONDS = 60.0
...
def _database_url() -> str:
    """Use only HISTORIA_DATABASE_URL; never fall back to Recepción DATABASE_URL."""
    env = _read_env_file()
    historia_url = _clean(os.getenv(ENV_KEY) or env.get(ENV_KEY), 4000)
    if not historia_url:
        return ""

    reception_url = _clean(
        os.getenv("DATABASE_URL") or env.get("DATABASE_URL"),
        4000,
    )
...
def _set_historia_search_path(conn) -> None:
    cur = conn.cursor()
    try:
        cur.execute('SET search_path TO "historia", public')
    finally:
...
def _connect():
    global _REMOTE_SCHEMA_READY_KEY
    url = _database_url()
    if not url:
    ...
    db_key = (cfg["host"].lower(), cfg["port"], cfg["database"], cfg["user"])
    try:
        if _REMOTE_SCHEMA_READY_KEY != db_key:
            with _REMOTE_SCHEMA_LOCK:
                if _REMOTE_SCHEMA_READY_KEY != db_key:
                    _ensure_remote_bridge_schema(conn)
                    _REMOTE_SCHEMA_READY_KEY = db_key
                else:
                    _set_historia_search_path(conn)
        else:
            _set_historia_search_path(conn)
        return conn
    except Exception:
...
def _doctor_presence_cached() -> dict:
    """Cache cloud presence; real LAN handoff still probes immediately on demand."""
    global _PRESENCE_CACHE, _PRESENCE_CACHE_MONOTONIC
    now_mono = time.monotonic()
    with _PRESENCE_CACHE_LOCK:
        elapsed = now_mono - _PRESENCE_CACHE_MONOTONIC
        if _PRESENCE_CACHE is not None and elapsed < _PRESENCE_CACHE_SECONDS:
            value = dict(_PRESENCE_CACHE)
            age = value.get("doctor_last_seen_age_seconds")
            if isinstance(age, (int, float)):
                adjusted = max(0, int(age + max(0.0, elapsed)))
                value["doctor_last_seen_age_seconds"] = adjusted
                value["doctor_online"] = adjusted <= 300
            value["presence_cached"] = True
            value["presence_cache_age_seconds"] = int(max(0.0, elapsed))
            return value

        value = dict(_doctor_presence())
        _PRESENCE_CACHE = dict(value)
        _PRESENCE_CACHE_MONOTONIC = time.monotonic()
        value["presence_cached"] = False
        value["presence_cache_age_seconds"] = 0
        return value
...
def bridge_status() -> dict:
    ...
    if configured:
        try:
            presence.update(_doctor_presence_cached())
        except Exception as exc:
