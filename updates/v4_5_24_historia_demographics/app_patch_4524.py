...
# v4.5.24 — Recepción envía a Historia los datos administrativos disponibles
# para pacientes NUEVOS: nacimiento, teléfono, correo y domicilio.
# No envía notas clínicas ni cambia el esquema de ninguna base.

import app_patch_4523 as previous
...
def v4524_create_visit_batch_payment(
    ...
    try:
        ...
        if patient:
            ...
            if not attention_type:
                attention_type = (
                    "Subsecuente" if type_code == "S"
                    else ("Nuevo" if type_code == "N" else "Consulta")
                )
...
def v4524_health(user=core.Depends(core.current_user)):
    return {
        "ok": True,
        "version": APP_VERSION,
        "historia_new_patient_demographics": True,
        "fields": ["birth_date", "phone", "email", "address"],
        "clinical_notes_shared": False,
