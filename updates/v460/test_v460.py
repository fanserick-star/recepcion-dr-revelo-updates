...
assert meta['version']=='4.3.60'
assert meta['base_version']=='4.3.59'
...
manifest_bytes=(ROOT/'update_manifest.json').read_bytes()
assert hashlib.sha256(manifest_bytes).hexdigest()==meta['manifest_sha256']
...
for m in ['.native-slot.occupied.confirmed','.native-slot.occupied.cancelled','.native-slot.occupied.pending','.native-slot.occupied.rescheduled']:
    assert m in consts['V460_OVERLAY_CSS']
...
print('V460_BEHAVIOR_OK',meta['app_size'],meta['app_sha256'])
