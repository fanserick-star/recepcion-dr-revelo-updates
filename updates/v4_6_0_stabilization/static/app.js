...
async function wakeFromIdle(){
  ...
  setConnectionBadge('syncing','Reanudando','Abriendo la copia local · Neon solo se usará si realmente hace falta…');
  wakePromise=(async()=>{
    try{
...
async function openPatient(id,source='general'){
  ...
  const fromAttentionSearch=source==='attention-search';
    ...
    try{
      ...
      box.innerHTML=usable.length?usable.map(p=>{
        ...
        if(typeof isHistoricalPatient==='function'&&isHistoricalPatient(p)){
          ...
          const period=historicalLastPeriod(p);
...
let agendaWhatsappReviewPanelOpen=false;
...
async function loadWhatsappAudioBlob(id){
