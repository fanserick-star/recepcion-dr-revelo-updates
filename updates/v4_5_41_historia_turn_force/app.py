...
def _v4539_consultation_turn(db, visit):
    """Usa exactamente la misma fuente de numeración que Recepción/Reportes."""
    ...
    patient_days = {}
    for row_visit, patient in rows:
        patient_days.setdefault(
            (row_visit.fecha, int(patient.id)), []
        ).append((row_visit, patient))

    helper = getattr(core, "_v4457_consultation_turns", None)
    if callable(helper):
        turns = helper(patient_days)
        return turns.get((visit.fecha, int(visit.patient_id)))

    # Respaldo equivalente si una base muy antigua no expone el helper.
    consultation_groups = []
    for (fecha, pid), items in patient_days.items():
        ids = [
            int(v.id) for v, _p in items
            if not str(getattr(v, "procedimiento", "") or "").strip()
        ]
        if ids:
            consultation_groups.append((min(ids), pid))
    ordered = [pid for _first, pid in sorted(consultation_groups)]
    try:
