...
def _send_payload(conn, payload: dict) -> None:
    action = str(payload.get("action") or "handoff").strip().lower()
    if action == "handoff":
        _v4524_ensure_cloud_new_patient(conn, payload)

    result = _v4524_send_payload_original(conn, payload)

    # v4.5.41: además del UPSERT por event_id, fuerza el turno exacto sobre
    # cualquier fila activa del mismo paciente. Esto corrige handoffs viejos
    # cuyo event_id cambió o cuya fila activa quedó de una versión anterior.
    if action == "handoff":
        reception_patient_id = _clean(payload.get("reception_patient_id"), 120)
        try:
            reception_turn = int(payload.get("reception_turn")) if payload.get("reception_turn") not in (None, "") else None
            ...
            reception_turn = None
        if reception_patient_id and reception_turn and reception_turn > 0:
            cur = conn.cursor()
            try:
                cur.execute(
                    """
                    UPDATE waiting_queue
                       SET reception_turn=%s, updated_at=%s
                     WHERE reception_patient_id=%s
                       AND status IN ('waiting','in_consultation')
                    """,
                    (reception_turn, _now(), reception_patient_id),
                )
            finally:
                cur.close()
    return result
