...
def previous_app():
 for z in sorted(BACKUP.glob("auto_antes_actualizacion_*.zip"),key=lambda p:p.stat().st_mtime,reverse=True):
  try:
   with zipfile.ZipFile(z) as q:
    if "app.py" in q.namelist():
     b=q.read("app.py")
     if hb(b) in APP_MAP:return b
  except Exception:pass
 raise RuntimeError("No hallé respaldo compatible v4.3.42–v4.3.45")
...
def main():
 old=previous_app(); name=APP_MAP[hb(old)]; final_app=patch(old,getpatch(name))
 ...
 if os.getenv("RP_V446_NO_EXEC")=="1":print("v4.3.46 reconstruida y verificada");return 0
 os.execv(sys.executable,[sys.executable,str(ROOT/"app.py"),*sys.argv[1:]])
if __name__=="__main__":
 try:raise SystemExit(main())
 except Exception as e:print("No se pudo completar v4.3.46:",e,file=sys.stderr);raise
