            ...
            with sqlite3.connect(outbox, timeout=8) as local:
                ...
                for row in rows:
                    ...
                    if has_consultation:
                        ...
                        wanted = {"N":"Nuevo","S":"Subsecuente"}.get(first_type, "Consulta")
                    else:
                        wanted = "Procedimiento"
                    if str(payload.get("attention_type") or "") == wanted:
                        continue
                    payload["attention_type"] = wanted
                    local.execute(
