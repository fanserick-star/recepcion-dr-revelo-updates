...
function patientNameCell(r,home=false,isNewOverride=null){
  ...
  return `<div class="patient-name-line"><a href="#" onclick="openPatient(${p.id},'home');return false">${esc(p.nombre||'')}</a>${home?n:''}${home?'':dataWarning(p,true)}</div>`;
}
...
function openModal(html){$('#modalBody').innerHTML=html;const box=$('#modal .modalbox');if(box)box.classList.toggle('modalbox-wide',String(html||'').includes('new-attention-start-modal'));$('#modal').classList.remove('hidden')}
...
async function openPatient(id,source='general'){
  ...
  const actionHead=canDelete?'<th class="delete-col">Acción</th>':'';
  const history=p.visits.length?`<div class="patient-history-wrap"><table class="patient-history-table"><thead><tr><th>Fecha</th><th>Estado</th><th>Atención</th><th>Valor</th>${actionHead}</tr></thead><tbody>${p.visits.map(v=>`<tr><td>${fmtDate(v.fecha)}</td><td>${esc(statusLabel(v.tipo))}</td><td>${serviceBadge(v)}</td><td><span class="money-pill">${money(v.valor)}</span></td>${canDelete?`<td><button class="danger ghost small-delete" onclick="deleteVisit(${v.id},${id})">Borrar</button></td>`:''}</tr>`).join('')}</tbody></table></div>`:'<div class="patient-history-empty">Todavía no tiene atenciones registradas desde 2026.</div>';
  const historicalBox=(!p.visits.length&&p.historical)?`<div class="patient-historical-summary"><div><span>PACIENTE ANTERIOR</span><b>${esc(historicalLastLabel(p))}</b></div><small>Figura en el archivo histórico ${esc(historicalYears(p))}. Las atenciones antiguas no se copian a los reportes de 2026.</small></div>`:'';
  const missing=missingPatientFields(p);
  const missingChip=missing.length?`<span class="patient-modal-warning">⚠ Datos incompletos</span>`:'';
  const deletePatientButton=canDelete?`<button class="danger ghost patient-delete-compact" onclick="deletePatient(${id},${p.visits.length})">🗑 Borrar paciente</button>`:'';
  openModal(`<div class="patient-profile-modal"><div class="patient-profile-header"><div class="patient-profile-title"><h2>${esc(p.nombre)}</h2><div><span>${p.visits.length} ${p.visits.length===1?'atención':'atenciones'} desde 2026</span>${missingChip}</div></div><div class="patient-profile-actions"><button class="primary" onclick="attentionFor(${id})">＋ Nueva atención</button><button class="patient-edit-btn" onclick="editPatient(${id},'${source}')">Editar datos</button><button class="patient-edit-btn" onclick="openPatientBillingPreference(${id})">Datos de factura</button>${deletePatientButton}</div></div>${historicalBox}<div class="history patient-history"><div class="patient-history-heading"><h3>Historial 2026</h3><span>Más reciente primero</span></div>${history}</div></div>`);
}
...
async function newAttention(){
  ...
  attentionSearchSeq++;attentionWeekAnchor=toISO(new Date());
  openModal(`<div class="new-attention-start-modal"><div class="modal-form-heading"><h2>Nueva atención</h2><p>Selecciona un paciente de la agenda o busca otro paciente.</p></div><div class="new-patient-callout"><div><b>¿Es un paciente nuevo?</b><div class="muted">Créalo aquí y continuarás directamente al registro de su atención.</div></div><button class="primary" onclick="newPatient(true)">+ Crear paciente nuevo</button></div><div class="attention-start-search"><label for="aSearch">Buscar otro paciente</label><input id="aSearch" class="search uppercase-search" autocomplete="off" autocapitalize="characters" spellcheck="false" placeholder="Cédula, apellidos y nombres, celular o correo" oninput="upperSearchInput(this);attentionSearch()"><small>La lista completa de pacientes no se carga: solo buscamos cuando escribes.</small></div><div id="attentionWeekBlock" class="attention-week-block"><div class="attention-week-head"><div><b>Agenda</b><span>Jueves, viernes y sábado</span></div><div class="attention-week-nav"><button type="button" title="Semana anterior" onclick="moveAttentionWeek(-1)">‹</button><button type="button" class="week-today" onclick="currentAttentionWeek()">Esta semana</button><button type="button" title="Semana siguiente" onclick="moveAttentionWeek(1)">›</button></div><div class="attention-week-range"><strong id="attentionWeekLabel"></strong><span id="attentionWeekConflict" class="attention-week-conflict-note hidden"></span></div></div><div id="attentionWeekCalendar" class="attention-week-calendar"><div class="attention-week-loading">Cargando agenda…</div></div></div><div id="aResults" class="results attention-results hidden"></div></div>`);
  loadAttentionWeek(false,attentionWeekAnchor);
  setTimeout(()=>$('#aSearch')?.focus(),0);
}
function pRow(p){
  if(isHistoricalPatient(p)){
    const prior=`<div class="last-visit historical"><span>Histórico</span><b>${esc(historicalYears(p))}</b></div>`;
    return `<div class="patient-row attention-patient historical-result"><div class="patient-main"><div><b>${esc(p.nombre)}</b><span class="historical-badge">PACIENTE ANTERIOR</span></div>${patientIdentityHtml(p,true)}${prior}</div><button onclick="activateHistoricalPatient(${p.historical_id},'attention')">Seleccionar</button></div>`;
  }
  const prior=p.ultima_atencion?`<div class="last-visit"><span>Última atención</span><b>${fmtDate(p.ultima_atencion)}</b></div>`:`<div class="last-visit new"><span>Paciente</span><b>Sin atenciones registradas</b></div>`;
  return `<div class="patient-row attention-patient"><div class="patient-main"><b>${esc(p.nombre)}</b>${patientIdentityHtml(p,true)}${dataWarning(p)}${prior}</div><button onclick="attentionFor(${p.id})">Seleccionar</button></div>`;
}
async function attentionSearch(){
  const input=$('#aSearch'),week=$('#attentionWeekBlock'),box=$('#aResults');if(!input||!box)return;
  const q=String(input.value||'').trim();
  clearTimeout(attentionSearchTimer);
  const seq=++attentionSearchSeq;
  if(!q){box.classList.add('hidden');box.innerHTML='';week?.classList.remove('hidden');return}
  week?.classList.add('hidden');box.classList.remove('hidden');
  if(q.length<2){box.innerHTML='<div class="attention-search-hint">Escribe al menos 2 caracteres para buscar.</div>';return}
  box.innerHTML='<div class="attention-search-hint">Buscando…</div>';
  attentionSearchTimer=setTimeout(async()=>{
    try{
      const d=await api('/api/patients?q='+encodeURIComponent(q)+'&limit=20');
      if(seq!==attentionSearchSeq)return;
      box.innerHTML=d.map(p=>pRow(p)).join('')||'<div class="attention-search-hint">No se encontraron pacientes.</div>';
    }catch(e){if(seq===attentionSearchSeq)box.innerHTML=`<div class="attention-search-hint err">${esc(e.message)}</div>`}
  },180);
}
