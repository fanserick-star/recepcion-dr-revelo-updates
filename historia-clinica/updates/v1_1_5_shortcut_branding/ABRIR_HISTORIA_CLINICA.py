...
LAUNCHER_VERSION = "1.1.5-shortcut-branding-1"
...
PROTECTED_FILES = {".env"}
MUTEX_NAME = "DrArmandoRevelo_HistoriaClinica_Launcher_v3"
...
def _ensure_windows_shortcuts() -> None:
    """Autorrepara accesos directos y obliga a que el arranque pase por INICIAR.bat."""
    if os.name != "nt":
        return
...
foreach ($dir in $dirs) {{
  ...
  Get-ChildItem -LiteralPath $dir -Filter '*.lnk' -ErrorAction SilentlyContinue |
    Where-Object {{
      $_.Name -ne $name -and (
        $_.Name -like 'Historia Clinica*Revelo*.lnk' -or
        $_.Name -like 'Historia Clínica*Revelo*.lnk' -or
        $_.Name -like 'Historia*Dr*Revelo*.lnk'
      )
...
"""
        ...
        if proc.returncode != 0:
            _log(f"Reparación de acceso directo terminó con código {proc.returncode}")
        else:
            try:
                ctypes.windll.shell32.SHChangeNotify(0x08000000, 0, None, None)
            except Exception:
                pass
            _write_state(shortcut_target="INICIAR.bat", shortcut_branding_ok=True)
    except Exception as exc:
...
def _ensure_dependencies(manifest:dict|None=None, force:bool=False, splash:Splash|None=None):
    if splash:splash.set(1,"Preparando Historia Clínica…","Comprobando Python y componentes")
    _create_venv(); py=_python_exe(False); wanted=_requirements_hash(); marker=_data_dir()/"requirements_state.json"; old={}
    ...
    if need:
        if splash:splash.set(1,"Preparando componentes…","Instalando o reparando dependencias")
        subprocess.run([str(py),"-m","pip","install","--disable-pip-version-check","--no-warn-script-location","-r",str(ROOT/"requirements.txt")],cwd=str(ROOT),check=True)
        for item in required:
            module=str(item.get("import") or "").strip(); spec=str(item.get("pip") or "").strip()
            if module and not _import_ok(py,module) and spec:
                subprocess.run([str(py),"-m","pip","install","--disable-pip-version-check",spec],cwd=str(ROOT),check=True)
...
def _check_and_apply_update(remote:dict|None,splash:Splash|None=None):
    local=_local_package_version(); coherent=_installation_consistent()
    if remote is None:
        return {"updated":False,"deferred":True,"local":local}
    rv=str(remote.get("version") or ""); should=_vtuple(rv)>_vtuple(local) or (rv==local and not coherent)
    if not should:return {"updated":False,"version":local}
    if coherent and _candidate_failure_count(remote)>=2:
        _log(f"Candidata {rv} en cuarentena")
        return {"updated":False,"quarantined":True,"version":local}
    stage=None;entries=[]
    try:
        stage,entries=_stage_update(remote,splash)
        if not entries:
            return {"updated":False,"version":local}
        _trial_runtime(remote,stage,entries,splash)
        result=_apply_staged(remote,stage,entries)
        result["version"]=rv; result["entries"]=[{"rel":e["rel"]} for e in entries]
        _clear_candidate_failure(); _write_state(last_update_version=rv,last_update_ok=True)
        return result
    except Exception as exc:
        count=_mark_candidate_failure(remote,exc); _log(f"Candidata {rv} falló preflight/aplicación #{count}: {exc}")
        return {"updated":False,"candidate_failed":True,"error":str(exc),"version":local}
    finally:
...
def _open_ui(port:int)->bool:
    ...
    try:
        ...
        try:
            window=webview.create_window(TITLE,url,width=1440,height=900,min_size=(1050,700),resizable=True,text_select=True,maximized=True); maximize=False
        except TypeError:
            window=webview.create_window(TITLE,url,width=1440,height=900,min_size=(1050,700),resizable=True,text_select=True); maximize=True
        if maximize:
            def on_start():
                try:window.maximize()
                except Exception:pass
    ...
    if edge:
        try:
            subprocess.Popen([str(edge),f"--app={url}","--start-maximized","--disable-background-mode","--no-first-run"],cwd=str(ROOT),creationflags=getattr(subprocess,"CREATE_NO_WINDOW",0));return True
        except Exception as exc:_log("Edge app falló: "+repr(exc))
    try:webbrowser.open(url,new=2);return True
    except Exception:return False
...
def main():
    ...
    try:
        ...
        if update_result.get("candidate_failed"):
            splash.set(2,"Actualización descartada de forma segura","La versión instalada seguirá funcionando")
            _log("Preflight rechazó candidata: "+str(update_result.get("error") or ""));time.sleep(.45)
        elif update_result.get("quarantined"):
            splash.set(2,"Actualización en cuarentena","Abriendo la última versión estable");time.sleep(.4)
        ...
        try:
            ...
            if update_result.get("updated") and update_result.get("backup"):
                ...
                _write_state(last_update_ok=False,rollback_after_startup=True,last_start_error=str(start_exc)[:1200])
