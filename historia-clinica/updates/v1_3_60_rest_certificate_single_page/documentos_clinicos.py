    ...
    try:
        conn.executescript(
            ...
            CREATE TABLE IF NOT EXISTS certificates(
              ...
              certificate_no TEXT,
              certificate_type TEXT NOT NULL DEFAULT 'medical',
        ...
        _ensure_columns("certificates", [
            ...
            ("rest_to","TEXT"),("body_text","TEXT"),("certificate_no","TEXT"),
            ("certificate_type","TEXT NOT NULL DEFAULT 'medical'"),("extra_json","TEXT NOT NULL DEFAULT '{}'"),
            ("created_at","TEXT"),("updated_at","TEXT"),("deleted_at","TEXT"),
        ])
...
def _next_certificate_no(conn: sqlite3.Connection) -> str:
    rows = conn.execute(
        "SELECT certificate_no FROM certificates WHERE COALESCE(deleted_at,'')='' AND COALESCE(certificate_no,'')<>''"
    ).fetchall()
    nums = []
    for row in rows:
        raw = str(row["certificate_no"] or "").strip().upper()
        m = re.search(r"(\d+)$", raw)
        if m:
            try:
                ...
                pass
    return f"CERT-{max([0, *nums]) + 1:06d}"
...
def _date_short_two_year(value: str) -> str:
    try:
        d = date.fromisoformat(str(value or "")[:10])
        return d.strftime("%d/%m/%y")
    except Exception:
    ...
    if isinstance(additional, list):
        for item in additional:
            if not isinstance(item, dict):
                continue
            item_dx = str(item.get("diagnosis") or "").strip().upper()
            item_cie = str(item.get("cie10") or "").strip().upper()
            if item_dx:
                pieces.append(item_dx + (f" CIE-10: {item_cie}" if item_cie else ""))
    elif adx:
...
def _certificate_body_html(body_text: str, patient_name: str, national_id: str) -> str:
    ...
    # Compatibilidad visual: unifica certificados antiguos y nuevos como "CIE-10:".
    safe = re.sub(r"\bCIE\s*-?\s*10\s*:?\s*", "CIE-10: ", safe, flags=re.IGNORECASE)
    ...
    # Resalta toda la fecha de atención en la frase principal.
    safe = re.sub(
        r"(EL D[IÍ]A\s+)(\d{1,2}\s+DE\s+[A-ZÁÉÍÓÚÑ]+\s+DEL\s+\d{4})",
        lambda m: m.group(1) + "<strong>" + m.group(2) + "</strong>",
        safe,
        count=1,
        flags=re.IGNORECASE,
    )
...
.config-status-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px}
.config-status-item{border:1px solid #d7e0ea;border-radius:11px;padding:13px 14px;display:flex;gap:10px;align-items:flex-start;background:#fff}
.config-status-item .mark{width:26px;height:26px;border-radius:50%;display:grid;place-items:center;background:#e8f5ee;color:#238451;font-weight:900;flex:none}
.config-status-item.pending .mark{background:#fff5dc;color:#a16a00}
.config-status-item.error .mark{background:#fdecec;color:#b42318}
.config-status-item b{display:block;color:#173b66}.config-status-item small{display:block;color:#64748b;margin-top:3px}
.config-tool-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px}
.config-tool-btn{border:1px solid #cbd5e1;border-radius:11px;background:#f8fafc;text-align:left;padding:14px;cursor:pointer;color:#173b66}
.config-tool-btn:hover{background:#edf3f9}.config-tool-btn:disabled{opacity:.65;cursor:wait}
.config-tool-btn b{display:block;font-size:14px}.config-tool-btn small{display:block;color:#64748b;margin-top:4px;line-height:1.3}
@media(max-width:850px){.config-status-grid,.config-tool-grid{grid-template-columns:1fr}}
...
<main class='sheet'>{one}{one}</main>
    ...
    def certificates_list(patient_id: str = ""):
        ...
        body_rows = "".join(
            f"<tr><td>{_e(r['issued_at'][:16].replace('T',' '))}</td><td>{_e(_row_value(r,'certificate_no',''))}</td><td>{_e(r['name'])}</td>"
            ...
            f"<td><a href='{'/certificados/reposo/'+_e(r['id'])+'/vista' if str(_row_value(r,'certificate_type','medical'))=='rest_isolation' else '/certificados/'+_e(r['id'])+'/vista'}' target='_blank'>Ver / imprimir</a></td></tr>"
            for r in rows
        ) or "<tr><td colspan='7'>No hay certificados emitidos.</td></tr>"
        ...
        <div class='docs-card'><table class='list-table'><thead><tr><th>Fecha</th><th>N.º</th><th>Paciente</th><th>Tipo</th><th>Diagnóstico</th><th>Reposo</th><th></th></tr></thead><tbody>{body_rows}</tbody></table></div></section>"""
    ...
    def certificate_new(patient_id: str, encounter_id: str = ""):
        ...
        <p>El texto se genera con los datos de la atención y puede editarse antes de emitir.</p>
        <div class='doc-actions'><a class='doc-light' href='/certificados/reposo/nuevo?patient_id={_e(patient_id)}&encounter_id={_e(encounter_id)}'>Usar certificado de reposo / aislamiento</a></div></div></div>
        ...
        <div id='cert-extra-dx-list' class='full' style='display:grid;grid-template-columns:1fr 1fr;gap:12px'></div>
        ...
        function additionalDiagnoses(){{
          return Array.from(document.querySelectorAll('.cert-extra-dx-row')).map(row=>({{
            diagnosis:row.querySelector('.cert-extra-dx')?.value||'',
            cie10:row.querySelector('.cert-extra-cie')?.value||''
          }})).filter(x=>String(x.diagnosis||'').trim() || String(x.cie10||'').trim());
        }}

        function certPayload(){{
          const extras=additionalDiagnoses();
          return {{
            ...
            additional_diagnosis:extras[0]?.diagnosis||'',additional_cie10:extras[0]?.cie10||'',
        ...
        const extraDxList=document.getElementById('cert-extra-dx-list');

        function addExtraDiagnosis(){{
          const row=document.createElement('div');
          row.className='cert-extra-dx-row';
          row.style.display='contents';

          const dxLabel=document.createElement('label');
          dxLabel.textContent='Diagnóstico adicional';
          const dxInput=document.createElement('input');
          dxInput.className='cert-extra-dx';
          dxLabel.appendChild(dxInput);

          const cieLabel=document.createElement('label');
          cieLabel.textContent='CIE-10 adicional';
          const cieInput=document.createElement('input');
          cieInput.className='cert-extra-cie';
          cieLabel.appendChild(cieInput);

          row.appendChild(dxLabel);
          row.appendChild(cieLabel);
          extraDxList.appendChild(row);

          [dxInput,cieInput].forEach(el=>{{
            el.addEventListener('input',()=>scheduleRegen(false));
            el.addEventListener('change',()=>scheduleRegen(false));
          }});
          dxInput.focus();
        }}

        addDxBtn?.addEventListener('click',addExtraDiagnosis);
        ['cert-dx','cert-cie','cert-proc','cert-date'].forEach(id=>{{
    ...
    async def certificate_text(request: Request):
        ...
        with _connect(db_path) as conn:
            ...
            if not isinstance(extras, list):
                extras = str(p.get("additional_diagnosis") or "")
            body_text = _certificate_body(
                patient, enc,
                str(p.get("diagnosis") or ""), str(p.get("cie10") or ""),
                extras, str(p.get("additional_cie10") or ""),
    ...
    async def certificate_save(request: Request):
        ...
        with _connect(db_path) as conn:
            ...
            stored_extra_dx = " | ".join(
                str(x.get("diagnosis") or "").strip().upper()
                for x in extras if isinstance(x, dict) and str(x.get("diagnosis") or "").strip()
            )
            stored_extra_cie = " | ".join(
                str(x.get("cie10") or "").strip().upper()
                for x in extras if isinstance(x, dict) and str(x.get("cie10") or "").strip()
            )
            issued_at = str(p.get("issued_date") or date.today().isoformat()) + "T" + datetime.now().strftime("%H:%M:%S")
            certificate_no = _next_certificate_no(conn)
            conn.execute(
                """INSERT INTO certificates(
                   id,patient_id,encounter_id,issued_at,diagnosis,cie10,additional_diagnosis,
                   additional_cie10,procedure_text,rest_days,rest_from,rest_to,body_text,certificate_no,
                   created_at,updated_at,deleted_at
                   ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NULL)""",
                (
                    cid, patient_id, str(p.get("encounter_id") or "") or None, issued_at,
                    str(p.get("diagnosis") or "").upper(), str(p.get("cie10") or "").upper(),
                    stored_extra_dx, stored_extra_cie,
                    str(p.get("procedure_text") or "").upper(), rest_days,
                    str(p.get("rest_from") or ""), str(p.get("rest_to") or ""),
                    body_text.upper(), certificate_no, stamp, stamp,
                ),
            )
            conn.commit()
...
<main class='sheet'>
  ...
  <div class='issue-date'><b>N.º {_e(_row_value(row,'certificate_no',''))}</b> · Quevedo, {_e(issue_date)}</div>
  ...
  <footer>
    ...
    @app.get("/certificados/reposo/nuevo", response_class=HTMLResponse)
    def rest_certificate_new(patient_id: str, encounter_id: str = ""):
        with _connect(db_path) as conn:
            patient_id = _canonical_patient_id(conn, patient_id)
            patient = _patient(conn, patient_id)
            if not patient:
                return HTMLResponse(base("Certificado de reposo", "<p>Paciente no encontrado.</p>", "certificados"), status_code=404)
            encounter = _encounter(conn, encounter_id)
            settings = _settings(conn)

        today = date.today().isoformat()
        dx = str((encounter["diagnosis"] if encounter else "") or "")
        clinical_note = str((encounter["clinical_note"] if encounter else "") or "")
        patient_address = str(_row_value(patient, "address") or "")
        patient_phone = str(_row_value(patient, "phone") or "")
        patient_email = str(_row_value(patient, "email") or "")
        national_id = str(_row_value(patient, "national_id") or "")
        history_no = str(_row_value(patient, "legacy_patient_id") or national_id or patient_id)
        establishment = f"{settings.get('center_name','Centro de Urología')} {settings.get('doctor_name','Dr. Armando Revelo')}".strip()

        body = f"""{_styles()}
        <section class='docs-wrap'>
          <div class='docs-head'><div>
            <span class='eyebrow'>CERTIFICADO DE REPOSO / AISLAMIENTO</span>
            <h1>{_e(_row_value(patient,'name'))}</h1>
            <p>Formato detallado para reposo médico. Los datos de la ficha se cargan automáticamente y pueden ajustarse antes de emitir.</p>
          ...
          <div class='docs-grid'>
            <div class='docs-card'>
              <h3>1.- Datos del establecimiento / usuario</h3>
              <div class='docs-fields'>
                <label class='full'>Establecimiento de salud<input id='rest-establishment' value='{_e(establishment)}'></label>
                <label>Lugar<input id='rest-place' value='Quevedo'></label>
                <label>Fecha de emisión<input id='rest-issued' type='date' value='{today}'></label>
                <label class='full'>Paciente<input value='{_e(_row_value(patient,"name"))}' readonly></label>
                <label>Institución / Empresa<input id='rest-institution' placeholder='Opcional'></label>
                <label>Puesto de trabajo<input id='rest-job' placeholder='Opcional'></label>
                <label>Cédula / Pasaporte<input value='{_e(national_id)}' readonly></label>
                <label>No. Historia Clínica<input id='rest-history' value='{_e(history_no)}'></label>
                <label class='full'>Dirección domiciliaria<input id='rest-address' value='{_e(patient_address)}'></label>
                <label>Teléfono<input id='rest-phone' value='{_e(patient_phone)}'></label>
                <label>Correo<input id='rest-email' value='{_e(patient_email)}'></label>
              </div>
            </div>

            <div class='docs-card'>
              <h3>2.- Motivo de aislamiento / enfermedad</h3>
              <div class='docs-fields'>
                <label>Diagnóstico<input id='rest-dx' value='{_e(dx)}'></label>
                <label>Código CIE-10<input id='rest-cie'></label>
                <div id='rest-extra-dx-list' class='full' style='display:grid;grid-template-columns:1fr 1fr;gap:12px'></div>
                <div class='full'><button type='button' class='doc-light' id='rest-add-dx' style='border:0;border-radius:9px;padding:9px 12px;font-weight:800;cursor:pointer'>+ Agregar diagnóstico adicional</button></div>
                <label class='full'>Descripción del motivo de reposo<input id='rest-reason' value='Se prescribe medicación pertinente y reposo médico domiciliario'></label>
                <label>Contingencia
                  <select id='rest-contingency'><option value=''>Seleccione…</option><option value='SI'>SI</option><option value='NO'>NO</option></select>
                </label>
                <label>Presenta síntomas
                  <select id='rest-symptoms'><option value=''>Seleccione…</option><option value='SI'>SI</option><option value='NO'>NO</option></select>
                </label>
                <label class='full'>Enfermedad / descripción clínica<textarea id='rest-disease' rows='5'>{_e(clinical_note)}</textarea></label>
                <label>Días de reposo<input id='rest-days' type='number' min='1' value='1'></label>
                <label>Desde<input id='rest-from' type='date' value='{today}'></label>
                <label>Hasta<input id='rest-to' type='date' value='{today}'></label>
              </div>
            </div>
          </div>
        ...
        function recalcRestTo(){{
          const days=Math.max(1,Number(document.getElementById('rest-days').value||1));
          const raw=document.getElementById('rest-from').value;if(!raw)return;
          const d=new Date(raw+'T12:00:00');if(Number.isNaN(d.getTime()))return;
          d.setDate(d.getDate()+days-1);
          document.getElementById('rest-to').value=localIso(d);
        }}
        document.getElementById('rest-days').addEventListener('input',recalcRestTo);
        document.getElementById('rest-from').addEventListener('change',recalcRestTo);

        const restExtraList=document.getElementById('rest-extra-dx-list');
        document.getElementById('rest-add-dx').addEventListener('click',()=>{{
          const row=document.createElement('div');
          row.className='rest-extra-dx-row';
          row.style.display='contents';

          const dxLabel=document.createElement('label');
          dxLabel.textContent='Diagnóstico adicional';
          const dxInput=document.createElement('input');
          dxInput.className='rest-extra-dx';
          dxLabel.appendChild(dxInput);

          const cieLabel=document.createElement('label');
          cieLabel.textContent='CIE-10 adicional';
          const cieInput=document.createElement('input');
          cieInput.className='rest-extra-cie';
          cieLabel.appendChild(cieInput);

          row.appendChild(dxLabel);row.appendChild(cieLabel);restExtraList.appendChild(row);
          dxInput.focus();
        }});

        function restAdditionalDiagnoses(){{
          return Array.from(document.querySelectorAll('.rest-extra-dx-row')).map(row=>({{
            diagnosis:row.querySelector('.rest-extra-dx')?.value||'',
            cie10:row.querySelector('.rest-extra-cie')?.value||''
          }})).filter(x=>String(x.diagnosis||'').trim()||String(x.cie10||'').trim());
        }}
        ...
        function payload(){{
          return {{
            ...
            issued_date:document.getElementById('rest-issued').value,
            establishment:document.getElementById('rest-establishment').value,
            place:document.getElementById('rest-place').value,
            institution:document.getElementById('rest-institution').value,
            job_title:document.getElementById('rest-job').value,
            history_no:document.getElementById('rest-history').value,
            ...
            diagnosis:document.getElementById('rest-dx').value,
            cie10:document.getElementById('rest-cie').value,
            additional_diagnoses:restAdditionalDiagnoses(),
            rest_reason:document.getElementById('rest-reason').value,
            contingency:document.getElementById('rest-contingency').value,
            symptoms:document.getElementById('rest-symptoms').value,
            disease_text:document.getElementById('rest-disease').value,
            rest_days:Number(document.getElementById('rest-days').value||1),
            rest_from:document.getElementById('rest-from').value,
            rest_to:document.getElementById('rest-to').value
          }};
        }}
        ...
        async function openRest(printNow=false){{
          try{{
            ...
            const w=window.open(j.preview_url,'_blank');
            if(printNow&&w)setTimeout(()=>w.print(),700);
        ...
        return HTMLResponse(base("Certificado de reposo / aislamiento", body, "certificados"))


    @app.post("/api/certificados/reposo")
    async def rest_certificate_save(request: Request):
        ...
        with _connect(db_path) as conn:
            ...
            rest_days = max(1, int(p.get("rest_days") or 1))
            contingency = str(p.get("contingency") or "").strip().upper()
            symptoms = str(p.get("symptoms") or "").strip().upper()
            if contingency not in {"SI","NO"}:
                return JSONResponse({"ok": False, "error": "Seleccione si existe contingencia."}, status_code=400)
            if symptoms not in {"SI","NO"}:
                return JSONResponse({"ok": False, "error": "Seleccione si el paciente presenta síntomas."}, status_code=400)
            extras = p.get("additional_diagnoses")
            if not isinstance(extras, list):
                extras = []
            extras = [
                {
                    "diagnosis": str(x.get("diagnosis") or "").strip().upper(),
                    "cie10": str(x.get("cie10") or "").strip().upper(),
                }
                for x in extras if isinstance(x, dict)
                and (str(x.get("diagnosis") or "").strip() or str(x.get("cie10") or "").strip())
            ]
            issued_at = str(p.get("issued_date") or date.today().isoformat()) + "T" + datetime.now().strftime("%H:%M:%S")
            certificate_no = _next_certificate_no(conn)
            extra = {
                "establishment": str(p.get("establishment") or ""),
                "place": str(p.get("place") or "Quevedo"),
                "institution": str(p.get("institution") or ""),
                "job_title": str(p.get("job_title") or ""),
                "history_no": str(p.get("history_no") or ""),
                "address": str(p.get("address") or ""),
                "phone": str(p.get("phone") or ""),
                "email": str(p.get("email") or ""),
                "rest_reason": str(p.get("rest_reason") or ""),
                "contingency": contingency,
                "symptoms": symptoms,
                "disease_text": str(p.get("disease_text") or ""),
                "additional_diagnoses": extras,
            }
            body_text = str(extra["disease_text"] or p.get("diagnosis") or "").strip().upper()
            stored_extra_dx = " | ".join(x["diagnosis"] for x in extras if x["diagnosis"])
            stored_extra_cie = " | ".join(x["cie10"] for x in extras if x["cie10"])
            conn.execute(
                """INSERT INTO certificates(
                   id,patient_id,encounter_id,issued_at,diagnosis,cie10,additional_diagnosis,
                   additional_cie10,procedure_text,rest_days,rest_from,rest_to,body_text,certificate_no,
                   certificate_type,extra_json,created_at,updated_at,deleted_at
                   ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NULL)""",
                (
                    cid, patient_id, str(p.get("encounter_id") or "") or None, issued_at,
                    str(p.get("diagnosis") or "").upper(), str(p.get("cie10") or "").upper(),
                    stored_extra_dx, stored_extra_cie, str(extra["rest_reason"] or "").upper(), rest_days,
                    str(p.get("rest_from") or ""), str(p.get("rest_to") or ""),
                    body_text, certificate_no, "rest_isolation", json.dumps(extra, ensure_ascii=False),
                    stamp, stamp,
                ),
            )
            conn.commit()
        return JSONResponse({"ok": True, "id": cid, "preview_url": f"/certificados/reposo/{cid}/vista"})


    @app.get("/certificados/reposo/{cert_id}/vista", response_class=HTMLResponse)
    def rest_certificate_preview(cert_id: str):
        with _connect(db_path) as conn:
            row = conn.execute(
                """SELECT c.*,p.name,p.national_id,p.legacy_patient_id
                   FROM certificates c JOIN patients p ON p.id=c.patient_id
                   WHERE c.id=?""",
                (cert_id,),
            ).fetchone()
        ...
        try:
            extra = json.loads(str(_row_value(row, "extra_json", "{}") or "{}"))
            if not isinstance(extra, dict):
                extra = {}
        except Exception:
            extra = {}

        issued = str(row["issued_at"] or "")[:10]
        rest_from = str(row["rest_from"] or "")
        rest_to = str(row["rest_to"] or "")
        rest_days = max(1, int(row["rest_days"] or 1))
        name = str(row["name"] or "")
        national_id = str(row["national_id"] or "")
        history_no = str(extra.get("history_no") or _row_value(row,"legacy_patient_id") or national_id or "")
        contingency = str(extra.get("contingency") or "").upper()
        symptoms = str(extra.get("symptoms") or "").upper()
        disease = str(extra.get("disease_text") or row["body_text"] or "")
        reason = str(extra.get("rest_reason") or row["procedure_text"] or "")
        certificate_no = str(_row_value(row, "certificate_no", "") or "")

        diagnoses = []
        primary_dx = str(row["diagnosis"] or "").strip().upper()
        primary_cie = str(row["cie10"] or "").strip().upper()
        if primary_dx or primary_cie:
            diagnoses.append({"diagnosis": primary_dx, "cie10": primary_cie})
        for item in extra.get("additional_diagnoses") or []:
            if isinstance(item, dict):
                dxv = str(item.get("diagnosis") or "").strip().upper()
                ciev = str(item.get("cie10") or "").strip().upper()
                if dxv or ciev:
                    diagnoses.append({"diagnosis": dxv, "cie10": ciev})
        diagnoses_html = "".join(
            f"<div class='diagnosis-line'><b>Diagnóstico {i}:</b> {_e(item['diagnosis'])}"
            + (f" <span><b>CIE-10:</b> {_e(item['cie10'])}</span>" if item['cie10'] else "")
            + "</div>"
            for i,item in enumerate(diagnoses,1)
        ) or "<div class='diagnosis-line'><b>Diagnóstico:</b> Sin diagnóstico registrado</div>"

        yes_box = "<span class='box'>X</span>"
        no_box = "<span class='box'>&nbsp;</span>"
        symptoms_yes = yes_box if symptoms == "SI" else no_box
        symptoms_no = yes_box if symptoms != "SI" else no_box
...
<meta charset='utf-8'>
<meta name='viewport' content='width=device-width,initial-scale=1'>
<title>Certificado de reposo · {_e(name)}</title>
<style>
*{{box-sizing:border-box}}
html,body{{margin:0;font-family:Arial,Helvetica,sans-serif;color:#111}}
body{{background:#e8edf1;padding:14px}}
.toolbar{{width:216mm;max-width:100%;margin:0 auto 8px}}
.toolbar button{{border:0;border-radius:8px;background:#173b66;color:#fff;padding:10px 15px;font-weight:800;cursor:pointer}}
.sheet{{width:216mm;min-height:279mm;margin:0 auto;background:#fff;padding:6mm 12mm 7mm;box-shadow:0 8px 28px #0f172a22}}
.brand-box{{height:34mm;border:1px solid #8fb3aa;border-radius:3mm;display:grid;grid-template-columns:24mm 1fr 27mm;align-items:center;gap:2mm;padding:1.7mm 3.5mm;position:relative;overflow:hidden}}
.brand-box:before,.brand-box:after{{content:"";position:absolute;right:-7mm;width:52mm;height:6mm;border-radius:50%;transform:rotate(16deg)}}
.brand-box:before{{top:-1mm;border-top:2.6mm solid #45972f}}.brand-box:after{{top:5mm;border-top:2.6mm solid #1595c6}}
.brand-logo{{width:19mm;height:19mm;object-fit:contain;justify-self:center;z-index:2}}
.brand-copy{{text-align:center;z-index:2;line-height:1.05}}.brand-copy h1{{margin:0;font-size:14.5pt;font-weight:900}}.brand-copy h2{{margin:.4mm 0;font-size:11.5pt}}
.brand-meta{{font-size:6.7pt;font-weight:700;line-height:1.1}}
.cert-no{{z-index:2;text-align:center;font-size:7.8pt;font-weight:900;color:#173b66;align-self:end;padding-bottom:2mm}}
.title{{text-align:center;font-size:12.7pt;font-weight:900;margin:3.5mm 0 3mm;color:#173b66}}
.section-title{{font-weight:900;font-size:10.3pt;margin:3mm 0 2mm;border-bottom:.45mm solid #111;padding-bottom:1mm}}
.row{{display:grid;grid-template-columns:1fr 1fr;gap:7mm;margin:1.8mm 0;font-size:10.2pt}}
.line{{margin:1.6mm 0;font-size:10pt;line-height:1.28}}
.line b,.row b{{font-weight:900}}
.diagnosis-line{{margin:1.5mm 0;font-size:10pt;line-height:1.28}}.diagnosis-line span{{margin-left:4mm}}
.box{{display:inline-grid;place-items:center;width:6.5mm;height:6.5mm;border:.35mm solid #333;margin:0 1.2mm;font-weight:900}}
.disease-box{{display:grid;grid-template-columns:auto 1fr;gap:3mm;align-items:start;margin:2mm 0}}
.disease-text{{border:.35mm solid #555;min-height:18mm;padding:2.4mm;white-space:pre-wrap;font-size:9.8pt;line-height:1.25}}
.rest-line{{display:grid;grid-template-columns:42mm 1fr;gap:4mm;align-items:end;margin:3.2mm 0;font-size:10.2pt}}
.rest-value{{border-bottom:.45mm solid #111;padding:0 2mm .8mm;text-align:center;font-weight:700}}
.hint{{font-size:7.1pt;text-align:center;margin-top:.5mm}}
.signature-area{{width:78mm;margin:4mm auto 2mm;text-align:center;font-size:8.8pt;color:#1f2937;page-break-inside:avoid;break-inside:avoid}}
.signature-space{{height:10mm}}.signature-line{{border-top:.45mm solid #111;margin-bottom:1.2mm}}
.signature-area .doctor{{font-weight:900;font-size:9.8pt}}.signature-label{{margin-top:.8mm;font-weight:800;font-size:7.8pt}}
.footer{{margin-top:2mm;border-top:.4mm solid #173b66;padding-top:1.5mm;font-size:7.1pt;text-align:center;color:#334155;page-break-inside:avoid;break-inside:avoid}}
@page{{size:Letter portrait;margin:0}}
@media print{{
  html,body{{width:216mm;height:279mm;background:#fff;padding:0}}
  .toolbar{{display:none}}
  .sheet{{margin:0;box-shadow:none;width:216mm;min-height:279mm;padding:5.5mm 12mm 6mm}}
  .signature-area,.footer{{page-break-inside:avoid;break-inside:avoid}}
}}
</style>
</head>
<body>
...
<main class='sheet'>
  <div class='brand-box'>
    ...
    <div class='brand-copy'>
      ...
      <div class='brand-meta'>
        MSP. {_e(settings.get("msp_registration"))} · Reg. Senescyt: {_e(settings.get("senescyt_registration"))}<br>
        {_e(settings.get("center_name"))} · {_e(settings.get("phones"))}
      </div>
    </div>
    <div class='cert-no'>N.º<br>{_e(certificate_no)}</div>
  </div>
  <div class='title'>CERTIFICADO MÉDICO DE REPOSO / AISLAMIENTO</div>

  <div class='section-title'>1.- DATOS DEL ESTABLECIMIENTO / USUARIO</div>
  <div class='line'><b>Establecimiento de salud:</b> {_e(extra.get("establishment") or settings.get("center_name"))}</div>
  <div class='row'><div><b>Lugar:</b> {_e(extra.get("place") or "Quevedo")}</div><div><b>Fecha de Emisión ({_e(_date_short_two_year(issued))}):</b></div></div>
  <div class='line'><b>Certifico que:</b> {_e(name.upper())}</div>
  <div class='line'><b>Institución / Empresa:</b> {_e(str(extra.get("institution") or "").upper())}</div>
  <div class='line'><b>Puesto de Trabajo:</b> {_e(str(extra.get("job_title") or "").upper())}</div>
  <div class='line'><b>No. Cédula de Identidad / Pasaporte:</b> {_e(national_id)}</div>
  <div class='line'><b>No. Historia Clínica:</b> {_e(history_no)}</div>
  <div class='line'><b>Dirección domiciliaria:</b> {_e(str(extra.get("address") or "").upper())}</div>
  <div class='row'><div><b>Teléfono:</b> {_e(extra.get("phone") or "")}</div><div><b>Correo:</b> {_e(extra.get("email") or "")}</div></div>

  <div class='section-title'>2.- MOTIVO DE AISLAMIENTO / ENFERMEDAD</div>
  {diagnoses_html}
  <div class='line'><b>DESCRIPCIÓN DEL MOTIVO DE REPOSO:</b> {_e(reason)}</div>
  <div class='line'><b>CONTINGENCIA:</b> {_e(contingency)}</div>
  <div class='line'><b>Presenta síntomas (marcar x):</b>&nbsp; SI {symptoms_yes} NO {symptoms_no}</div>
  <div class='disease-box'><div><b>Enfermedad (marcar x):</b> <span class='box'>X</span></div><div class='disease-text'>{_e(disease)}</div></div>

  <div class='rest-line'><b>Total, de días certificados:</b><div><div class='rest-value'>{rest_days} ({_e(_number_words_es(rest_days))}) {'DÍA' if rest_days==1 else 'DÍAS'}</div><div class='hint'>(En números y letras)</div></div></div>
  <div class='rest-line'><b>Desde ({_e(_date_short_two_year(rest_from))}):</b><div><div class='rest-value'>{_e(_certificate_rest_date(rest_from))}</div><div class='hint'>(En números y letras)</div></div></div>
  <div class='rest-line'><b>Hasta ({_e(_date_short_two_year(rest_to))}):</b><div><div class='rest-value'>{_e(_certificate_rest_date(rest_to))}</div><div class='hint'>(En números y letras)</div></div></div>
  ...
  <div class='footer'>
    {_e(settings.get("address"))} · {_e(settings.get("phones"))} · {_e(settings.get("email"))}
  </div>
</main>
</body>
</html>""")
    ...
    def configuration():
        ...
        <div class='config-nav'><a href='#consultorio'>Datos del consultorio</a><a href='#usuarios'>Usuarios</a><a href='#impresoras'>Impresoras</a><a href='#estado'>Estado y conexión</a><a href='#herramientas'>Herramientas técnicas</a><a href='#otros'>Otros ajustes</a></div>
        ...
        <section class='docs-card' id='estado'>
          <h2>Estado y conexión</h2>
          <p>Información técnica de la nube y del enlace privado con Recepción.</p>
          <div class='config-status-grid'>
            <div class='config-status-item' id='cfg-cloud-card'><span class='mark' id='cfg-cloud-mark'>✓</span><span><b>Nube</b><small id='cfg-cloud-text'>{_e(sync.get('message') or sync.get('state') or 'Comprobando…')}</small></span></div>
            <div class='config-status-item pending' id='cfg-lan-card'><span class='mark' id='cfg-lan-mark'>•</span><span><b>LAN</b><small id='cfg-lan-text'>Comprobando enlace con Recepción…</small></span></div>
            <div class='config-status-item'><span class='mark'>✓</span><span><b>Base local</b><small>Activa y disponible en esta PC.</small></span></div>
            <div class='config-status-item'><span class='mark'>✓</span><span><b>Sincronización</b><small id='cfg-sync-meta'>Pendientes: {_e(sync.get('pending',0))} · Última: {_e(sync.get('last_sync','') or 'Sin registro')}</small></span></div>
          </div>
          <div class='doc-actions'><button type='button' class='doc-primary' id='cfg-sync-now'>Sincronizar ahora</button></div>
        </section>

        <section class='docs-card' id='herramientas'>
          <h2>Herramientas técnicas</h2>
          <p>Estas opciones son para mantenimiento del programa y normalmente no hacen falta durante una consulta.</p>
          <div class='config-tool-grid'>
            <button type='button' class='config-tool-btn' id='cfg-repair-shortcut'><b>Crear / reparar acceso directo</b><small>Corrige el icono de Windows y el acceso de la barra de tareas.</small></button>
        ...
        function cfgNotify(message,type='success'){{
          if(typeof showAppToast==='function')showAppToast(message,type);
          else alert(message);
        }}
        function cfgSetState(card,mark,text,state,message){{
          if(!card||!mark||!text)return;
          card.classList.remove('pending','error');
          if(state==='pending')card.classList.add('pending');
          if(state==='error')card.classList.add('error');
          mark.textContent=state==='error'?'!':(state==='pending'?'•':'✓');
          text.textContent=message||'';
        }}
        async function cfgRefreshStatus(){{
          try{{
            const r=await fetch('/api/sync/status?t='+Date.now(),{{cache:'no-store'}});
            if(r.ok){{
              const s=await r.json();
              const card=document.getElementById('cfg-cloud-card'),mark=document.getElementById('cfg-cloud-mark'),txt=document.getElementById('cfg-cloud-text');
              const ok=s.state==='synced'&&s.online;
              const err=s.state==='offline';
              cfgSetState(card,mark,txt,err?'error':(ok?'ok':'pending'),s.message||(ok?'Sincronizada':'Sincronizando…'));
              const meta=document.getElementById('cfg-sync-meta');
              if(meta)meta.textContent='Pendientes: '+Number(s.pending||0).toLocaleString('es-EC')+' · Última: '+(s.last_sync||'Sin registro');
            }}
          }}catch(_e){{}}
          try{{
            const r=await fetch('/api/lan/status?t='+Date.now(),{{cache:'no-store'}});
            if(r.ok){{
              const s=await r.json();
              const card=document.getElementById('cfg-lan-card'),mark=document.getElementById('cfg-lan-mark'),txt=document.getElementById('cfg-lan-text');
              if(!s.ok)cfgSetState(card,mark,txt,'error',s.last_error||'No se pudo iniciar la LAN');
              else if(s.last_reception_seen)cfgSetState(card,mark,txt,'ok','Recepción conectada');
              else cfgSetState(card,mark,txt,'pending',s.firewall_rule===false?'Servicio activo · falta permiso de Windows':'Servicio activo · esperando Recepción');
            }}
          }}catch(_e){{}}
        }}
        async function cfgAction(id,url,success){{
          ...
          btn.addEventListener('click',async()=>{{
            ...
            try{{
              ...
              if(!r.ok||!d.ok)throw new Error(d.message||d.error||'No se pudo completar la acción');
              cfgNotify(d.message||success,'success');
              setTimeout(cfgRefreshStatus,500);
            }}catch(err){{cfgNotify(err.message||String(err),'error')}}
            finally{{btn.disabled=false;btn.innerHTML=old}}
          }});
        }}
        cfgAction('cfg-sync-now','/api/sync/now','Sincronización solicitada');
        cfgAction('cfg-repair-shortcut','/api/system/shortcut','Acceso directo reparado');
        cfgAction('cfg-repair-lan','/api/system/lan-firewall','Permiso LAN solicitado');
        cfgAction('cfg-open-folder','/api/system/open-folder','Carpeta abierta');
        cfgRefreshStatus();
        setInterval(cfgRefreshStatus,5000);
        </script>"""
