    ...
    def prescription_preview(rx_id: str):
        ...
        meds = "".join(
            (
                ...
                + (
                    f"<span><b>Dosis:</b> {_e(x.get('dose'))}</span>"
                    if str(x.get("dose") or "").strip() else ""
                )
                + (
                    f"<span><b>Frecuencia:</b> {_e(x.get('frequency'))}</span>"
                    if str(x.get("frequency") or "").strip() else ""
                )
                + (
                    f"<span><b>Duración:</b> {_e(x.get('duration'))}</span>"
                    if str(x.get("duration") or "").strip() else ""
                )
                + "</div></div>"
            )
        ...
        allergy_text = str(row["allergies"] or "NO").strip().upper()
        if allergy_detail:
            allergy_text += " · " + allergy_detail.upper()
...
<style>
:root{{--blue:#1393bf;--deep:#122244;--green:#4d982f;--ink:#111827;}}
*{{box-sizing:border-box}}
html,body{{margin:0;min-height:100%;font-family:Arial,Helvetica,sans-serif;color:var(--ink)}}
body{{background:#dde2e8;padding:16px}}
.toolbar{{width:148mm;max-width:100%;margin:0 auto 10px;display:flex;gap:8px;align-items:center;flex-wrap:wrap}}
...
.sheet{{
  width:148mm;min-height:210mm;margin:0 auto;background:#fff;position:relative;overflow:hidden;
  padding:7mm 8mm 7mm;box-shadow:0 8px 28px #0f172a2c;
}}
.brand-box{{
  min-height:42mm;border:1px solid #7ba8a0;border-radius:4mm 4mm 2mm 2mm;
  position:relative;display:grid;grid-template-columns:25mm 1fr;gap:4mm;align-items:center;
  padding:4mm 5mm 4mm 4mm;overflow:hidden;background:#fff;
}}
.brand-box:before,.brand-box:after{{
  content:"";position:absolute;right:-16mm;width:62mm;height:9mm;border-radius:50%;
  transform:rotate(16deg);pointer-events:none
}}
.brand-box:before{{top:-1mm;border-top:4mm solid var(--green)}}
.brand-box:after{{top:5mm;border-top:4mm solid var(--blue)}}
.brand-logo{{width:24mm;height:24mm;object-fit:contain;position:relative;z-index:2}}
.brand-copy{{position:relative;z-index:2;text-align:center;padding-right:8mm}}
.brand-copy h1{{margin:0;color:#0f172a;font-size:18pt;line-height:1}}
.brand-copy h2{{margin:1mm 0 0;color:#111827;font-size:15pt;line-height:1}}
.brand-copy .senescyt{{margin-top:2mm;font-family:Georgia,serif;font-size:10.5pt;font-weight:700}}
.brand-copy .services{{margin-top:1.2mm;font-family:Georgia,serif;font-size:9pt;font-weight:700;line-height:1.15}}
.patient-block{{margin-top:5mm;font-size:10.5pt}}
.line-row{{display:grid;grid-template-columns:auto 1fr;align-items:end;gap:2mm;margin:2.6mm 0}}
.line-row .label,.small-grid .label,.dx-grid .label{{font-weight:800;white-space:nowrap}}
.line{{border-bottom:1.5px solid #27364a;min-height:5.5mm;padding:0 1mm .7mm;font-weight:700}}
.small-grid{{display:grid;grid-template-columns:auto 26mm auto 45mm auto 1fr;gap:2mm;align-items:end;margin:2.7mm 0}}
.inline-value{{border-bottom:1.5px solid #27364a;min-height:5.5mm;padding:0 1mm .7mm;font-weight:700}}
.dx-grid{{display:grid;grid-template-columns:auto 1fr auto 28mm;gap:2mm;align-items:end;margin:2.7mm 0}}
.allergy-series{{display:flex;justify-content:space-between;align-items:flex-end;margin:3mm 0 1mm}}
.allergy{{font-weight:700}}
.series{{font-size:13pt;color:#3185a6;font-weight:800;letter-spacing:.4px}}
.rp-title{{font-family:Georgia,serif;font-size:19pt;font-weight:800;margin:2mm 0 2mm}}
.rp{{min-height:88mm;padding:1mm 2mm 0;font-size:11pt;line-height:1.35}}
.med-item{{margin-bottom:4mm}}
.med-name{{font-weight:900;font-size:12.5pt;letter-spacing:.1px}}
.med-instructions{{display:flex;gap:4mm;flex-wrap:wrap;margin:1mm 0 0 4mm;font-size:10.5pt}}
.med-instructions span{{white-space:nowrap}}
.med-empty{{color:#94a3b8;font-style:italic}}
.extra-instructions{{margin-top:4mm;padding-top:2mm;border-top:1px dashed #cbd5e1;white-space:pre-wrap}}
.watermark{{position:absolute;left:50%;bottom:27mm;transform:translateX(-50%);width:80mm;opacity:.075;filter:saturate(.75);pointer-events:none}}
.footer{{position:absolute;left:8mm;right:8mm;bottom:6mm;text-align:center;font-family:Georgia,serif;font-weight:700;font-size:8.5pt;line-height:1.25}}
.footer-line{{height:3mm;position:absolute;left:0;right:0;bottom:-6mm;background:linear-gradient(90deg,var(--green) 0 55%,var(--blue) 55% 100%)}}
@page{{size:A5 portrait;margin:0}}
@media print{{
  html,body{{width:148mm;height:210mm;background:#fff;padding:0}}
  .toolbar{{display:none!important}}
  .sheet{{box-shadow:none;margin:0;width:148mm;min-height:210mm;max-height:210mm}}
}}
</style>
</head>
<body>
<div class='toolbar'>
  ...
  <span>Formato A5 · una receta por hoja</span>
</div>
<main class='sheet'>
  <section class='brand-box'>
    <img class='brand-logo' src='/static/doctor_logo.png' alt='Logo'>
    <div class='brand-copy'>
      <h1>{_e(settings.get('doctor_name'))}</h1>
      <h2>{_e(settings.get('specialty') or 'Urólogo')}</h2>
  ...
  <section class='patient-block'>
    <div class='line-row'><span class='label'>Fecha:</span><span class='line'>{_e(issued_date)}</span></div>
    <div class='line-row'><span class='label'>Nombre:</span><span class='line'>{_e(row['name'])}</span></div>

    <div class='small-grid'>
      <span class='label'>Sexo:</span><span class='inline-value'>{_e(row['sex'])}</span>
      <span class='label'>Cédula:</span><span class='inline-value'>{_e(row['national_id'])}</span>
      <span class='label'>Edad:</span><span class='inline-value'>{_e(_age(row['birth_date']))}</span>
    </div>

    <div class='dx-grid'>
      <span class='label'>Diagnóstico:</span><span class='inline-value'>{_e(row['diagnosis'])}</span>
      <span class='label'>CIE 10:</span><span class='inline-value'>{_e(row['cie10'])}</span>
    </div>

    <div class='allergy-series'>
      <div class='allergy'>Alergias: {_e(allergy_text)}</div>
  ...
  <section class='rp'>
    {meds}
    {f"<div class='extra-instructions'>{_e(instructions).replace(chr(10),'<br>')}</div>" if instructions else ""}
  </section>

  <img class='watermark' src='/static/doctor_logo.png' alt=''>
  <footer class='footer'>
    {_e(settings.get('prescription_footer'))}
