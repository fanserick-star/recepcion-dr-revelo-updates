...
def install() -> None:
    ...
    if getattr(cls, "_v121_presence_installed", False):
        return
    cls._v121_presence_installed = True
    ...
    def start(self):
        ...
        thread = getattr(self, "_v121_presence_thread", None)
        if getattr(self, "enabled", False) and getattr(self, "url", "") and not (thread and thread.is_alive()):
            ...
            self._v121_presence_thread = thread
            thread.start()
        return result
