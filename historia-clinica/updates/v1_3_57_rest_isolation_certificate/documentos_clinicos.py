  ...
  <footer>
    ...
    def rest_certificate_new(patient_id: str, encounter_id: str = ""):
          ...
          <div class='docs-grid'>
            ...
            <div class='docs-card'>
              ...
              <div class='docs-fields'>
                ...
                <label>Contingencia
                  <select id='rest-contingency'><option value='SI'>SI</option><option value='NO'>NO</option></select>
                </label>
                <label>Presenta síntomas
                  <select id='rest-symptoms'><option value='SI'>SI</option><option value='NO'>NO</option></select>
                </label>
        ...
        async function saveRest(){{
          const r=await fetch('/api/certificados/reposo',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify(payload())}});
          const j=await r.json();
    ...
    async def rest_certificate_save(request: Request):
        ...
        with _connect(db_path) as conn:
            ...
            extra = {
                ...
                "contingency": str(p.get("contingency") or "SI").upper(),
                "symptoms": str(p.get("symptoms") or "SI").upper(),
            ...
            conn.execute(
                ...
                (
                    cid, patient_id, str(p.get("encounter_id") or "") or None, issued_at,
                    str(p.get("diagnosis") or "").upper(), str(p.get("cie10") or "").upper(),
                    "", "", str(extra["rest_reason"] or "").upper(), rest_days,
                    str(p.get("rest_from") or ""), str(p.get("rest_to") or ""),
    ...
    def rest_certificate_preview(cert_id: str):
        ...
        contingency = str(extra.get("contingency") or "SI").upper()
        symptoms = str(extra.get("symptoms") or "SI").upper()
...
.sheet{{width:216mm;min-height:279mm;margin:0 auto;background:#fff;padding:10mm 12mm 14mm;box-shadow:0 8px 28px #0f172a22}}
.title{{text-align:center;font-size:13pt;font-weight:900;margin:0 0 7mm}}
.section-title{{font-weight:900;font-size:11pt;margin:5mm 0 3mm;border-bottom:.45mm solid #111;padding-bottom:1.5mm}}
.row{{display:grid;grid-template-columns:1fr 1fr;gap:7mm;margin:2.5mm 0;font-size:10.5pt}}
.line{{margin:2.5mm 0;font-size:10.5pt;line-height:1.35}}
...
.footer{{margin-top:12mm;border-top:.4mm solid #173b66;padding-top:3mm;font-size:8.5pt;text-align:center;color:#334155}}
@page{{size:Letter portrait;margin:0}}
...
<main class='sheet'>
  ...
  <div class='line'><b>Diagnóstico:</b> {{_e(str(row["diagnosis"] or "").upper())}}</div>
  <div class='line'><b>Código CIE-10:</b> {{_e(str(row["cie10"] or "").upper())}}</div>
  ...
  <div class='footer'>
    <b>{{_e(settings.get("doctor_name"))}} · {{_e(settings.get("specialty"))}}</b><br>
    MSP. {{_e(settings.get("msp_registration"))}} · Reg. Senescyt: {{_e(settings.get("senescyt_registration"))}}<br>
    {{_e(settings.get("address"))}} · {{_e(settings.get("phones"))}}
  </div>
</main>
</body>
</html>""")
