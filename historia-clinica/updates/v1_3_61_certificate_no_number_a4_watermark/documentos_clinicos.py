...
<main class='sheet'>{one}{one}</main>
    ...
    def certificates_list(patient_id: str = ""):
        ...
        <div class='docs-card'><table class='list-table'><thead><tr><th>Fecha</th><th>Paciente</th><th>Tipo</th><th>Diagnóstico</th><th>Reposo</th><th></th></tr></thead><tbody>{body_rows}</tbody></table></div></section>"""
        ...
        with _connect(db_path) as conn:
            ...
            conn.execute(
                ...
                (
                    ...
                    str(p.get("rest_from") or ""), str(p.get("rest_to") or ""),
                    body_text, "rest_isolation", json.dumps(extra, ensure_ascii=False),
                    stamp, stamp,
                ),
            )
            conn.commit()
...
.toolbar{{width:210mm;max-width:100%;margin:0 auto 8px}}
.toolbar button{{border:0;border-radius:8px;background:#173b66;color:#fff;padding:10px 15px;font-weight:800;cursor:pointer}}
.sheet{{width:210mm;height:297mm;min-height:297mm;margin:0 auto;background:#fff;padding:6mm 12mm 58mm;box-shadow:0 8px 28px #0f172a22;position:relative;overflow:hidden}}
.brand-box{{height:34mm;border:1px solid #8fb3aa;border-radius:3mm;display:grid;grid-template-columns:24mm 1fr 24mm;align-items:center;gap:2mm;padding:1.7mm 3.5mm;position:relative;overflow:hidden;z-index:2;background:#fff}}
...
.signature-area{{position:absolute;left:50%;transform:translateX(-50%);bottom:21mm;width:88mm;text-align:center;font-size:8.9pt;color:#1f2937;page-break-inside:avoid;break-inside:avoid;z-index:2}}
.signature-space{{height:22mm}}.signature-line{{border-top:.45mm solid #111;margin-bottom:1.5mm}}
.signature-area .doctor{{font-weight:900;font-size:10pt}}.signature-label{{margin-top:1mm;font-weight:800;font-size:8pt}}
.footer{{position:absolute;left:12mm;right:12mm;bottom:4.5mm;border-top:.4mm solid #173b66;padding-top:1.5mm;font-size:7.1pt;text-align:center;color:#334155;page-break-inside:avoid;break-inside:avoid;z-index:2}}
.watermark{{position:absolute;left:50%;top:57%;transform:translate(-50%,-50%);width:70mm;height:auto;opacity:.075;filter:saturate(.85);pointer-events:none;z-index:0}}
.section-title,.row,.line,.diagnosis-line,.disease-box,.rest-line{{position:relative;z-index:2}}
@page{{size:A4 portrait;margin:0}}
@media print{{
  html,body{{width:210mm;height:297mm;background:#fff;padding:0}}
  .toolbar{{display:none}}
  .sheet{{margin:0;box-shadow:none;width:210mm;height:297mm;min-height:297mm;padding:5.5mm 12mm 58mm}}
