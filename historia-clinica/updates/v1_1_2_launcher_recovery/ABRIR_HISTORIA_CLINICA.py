...
class _Splash:
    """Pantalla sencilla de arranque. Nunca debe impedir que la app abra."""
    def __init__(self):
        self.root = None
        self.bar = None
        self.title = None
        ...
        try:
            ...
            head = tk.Frame(outer, bg="#123f73", height=70)
            head.pack(fill="x")
            ...
            if logo_path.is_file():
                try:
                    ...
                    if img.width() > 62 or img.height() > 62:
                        factor = max(1, int(max(img.width()/54, img.height()/54)))
                        img = img.subsample(factor, factor)
                    self._logo_img = img
                    tk.Label(head, image=img, bg="#123f73").pack(side="left", padx=(20,10), pady=8)
                except Exception as exc:
                    _log("Logo del splash no disponible: " + repr(exc))
            brand = tk.Frame(head, bg="#123f73")
            brand.pack(side="left", fill="y", pady=10)
            ...
            body = tk.Frame(outer, bg="#f4efe5", padx=26, pady=17)
            body.pack(fill="both", expand=True)
            self.title = tk.Label(body, text="Preparando Historia Clínica…", font=("Segoe UI", 13, "bold"), fg="#173b66", bg="#f4efe5")
            self.title.pack(anchor="w")
            self.detail = tk.Label(body, text="Comprobando el sistema", font=("Segoe UI", 9), fg="#655d52", bg="#f4efe5")
            self.detail.pack(anchor="w", pady=(4, 13))
            track = tk.Frame(body, bg="#ded6c9", height=8)
            track.pack(fill="x")
            track.pack_propagate(False)
            self.bar = tk.Frame(track, bg="#2b6aa7", height=8)
            self.bar.place(relx=0, rely=0, relheight=1, relwidth=.08)
    ...
    def set_stage(self, stage: int, title: str, detail: str = "") -> None:
        ...
        try:
            ...
            if self.bar: self.bar.place_configure(relwidth=max(.08, self.stage / 4.0))
            self.pump()
        except Exception as exc:
            _log("Splash set_stage: " + repr(exc))

    def pump(self) -> None:
        if not self.root:
            return
        try:
            secs = max(0, int(time.monotonic() - self.started))
            if self.elapsed:
                self.elapsed.config(text=f"{secs} s · etapa {self.stage} de 4" if secs >= 2 else f"Etapa {self.stage} de 4")
            self.root.update_idletasks()
            self.root.update()
        except Exception as exc:
            _log("Splash pump: " + repr(exc))
...
def _already_running() -> bool:
    return bool(_api_version(timeout=0.35))


def _relaunch_after_update() -> None:
    """Abre el launcher ya actualizado y termina el proceso que aplicó el cambio."""
    pyw = _venv_python(True)
    py = pyw if pyw.is_file() else _venv_python(False)
    flags = _hidden_flags()
    if os.name == "nt":
        flags |= getattr(subprocess, "CREATE_NEW_PROCESS_GROUP", 0)
    subprocess.Popen(
        [str(py), str(ROOT / "ABRIR_HISTORIA_CLINICA.py")],
...
def main() -> None:
    ...
    try:
        if _already_running():
            splash.set_stage(4, "Historia Clínica ya está iniciada", "Abriendo la ventana")
            time.sleep(0.15)
            splash.close()
        ...
        changed = _check_and_update(splash=splash)
        if changed:
            # Importante: no seguimos ejecutando código viejo después de reemplazar
            # el launcher. La siguiente instancia usa inmediatamente el archivo nuevo.
            splash.set_stage(3, "Actualización instalada", "Reiniciando el launcher")
            _log("Reiniciando launcher después de actualización")
            time.sleep(0.25)
            splash.close()
