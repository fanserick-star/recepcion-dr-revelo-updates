...
def install(app, context: dict) -> None:
    ...
    def prescription_preview(rx_id: str, print_now: int = 0):
        with _connect(db_path) as conn:
            row = conn.execute(
...
{"<script>window.addEventListener('load',()=>setTimeout(()=>window.print(),250));</script>" if print_now else ""}
</body>
</html>"""
    ...
    def certificate_preview(cert_id: str, print_now: int = 0):
        with _connect(db_path) as conn:
            row = conn.execute(
...
{"<script>window.addEventListener('load',()=>setTimeout(()=>window.print(),250));</script>" if print_now else ""}
    ...
    def rest_certificate_preview(cert_id: str, print_now: int = 0):
        with _connect(db_path) as conn:
            row = conn.execute(
