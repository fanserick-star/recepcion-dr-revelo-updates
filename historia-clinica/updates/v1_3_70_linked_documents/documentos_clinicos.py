...
{"<script>(()=>{let fired=false;const go=()=>{if(fired)return;fired=true;try{window.focus()}catch(_e){};setTimeout(()=>{try{window.print()}catch(_e){}},450)};if(document.readyState==='complete')go();else window.addEventListener('load',go,{once:true});window.addEventListener('pageshow',go,{once:true});})();</script>" if print_now else ""}
    ...
    def certificate_new(patient_id: str, encounter_id: str = "", queue_id: str = "", encounter_date: str = "", encounter_time: str = "", from_consultation: int = 0):
        ...
        function notifyCertParent(encId){{try{{if(window.opener&&!window.opener.closed)window.opener.postMessage({{type:'clinical-document-saved',encounter_id:encId||''}},location.origin)}}catch(_e){{}}}}
...
{"<script>(()=>{let fired=false;const go=()=>{if(fired)return;fired=true;try{window.focus()}catch(_e){};setTimeout(()=>{try{window.print()}catch(_e){}},450)};if(document.readyState==='complete')go();else window.addEventListener('load',go,{once:true});window.addEventListener('pageshow',go,{once:true});})();</script>" if print_now else ""}
    ...
    def rest_certificate_new(patient_id: str, encounter_id: str = "", queue_id: str = "", encounter_date: str = "", encounter_time: str = "", from_consultation: int = 0):
        ...
        function notifyRestParent(encId){{try{{if(window.opener&&!window.opener.closed)window.opener.postMessage({{type:'clinical-document-saved',encounter_id:encId||''}},location.origin)}}catch(_e){{}}}}
