...
ROOT = pathlib.Path(tempfile.gettempdir()) / "hc_runtime_smoke_132"
if ROOT.exists():
    shutil.rmtree(ROOT)
...
parts = [
    REPO / "historia-clinica/updates/v1_3_2_new_patient_confirm/app.py.part01",
    ...
    REPO / "historia-clinica/updates/v1_3_2_new_patient_confirm/app.py.part05",
]
...
shutil.copy2(
    REPO / "historia-clinica/updates/v1_3_2_new_patient_confirm/cloud_presence_patch.py",
    ROOT / "cloud_presence_patch.py",
)
shutil.copy2(
    REPO / "historia-clinica/updates/v1_3_2_new_patient_confirm/lan_bridge.py",
    ROOT / "lan_bridge.py",
)
...
assert app.APP_VERSION == "1.3.2", app.APP_VERSION
...
service = app.lan_bridge.LanService(ROOT, ROOT / "data" / "historia_clinica.db", "1.3.2", None)
...
# NUEVO from Reception: LAN payload fills the ficha and the doctor must
# confirm it before the consultation can open.
service = app.lan_bridge.LanService(ROOT, ROOT / "data" / "historia_clinica.db", "1.3.2", None)
lan_new = service.accept_handoff(
    {
        "event_id": "event-new-patient",
        "reception_patient_id": "rp-new-777",
        "display_name": "GUTIERREZ TROYA REINALDO PORFIRIO",
        "identification": "1717171717",
        "attention_type": "Nuevo",
        "birth_date": "1985-06-14",
        "phone": "0991234567",
        "email": "paciente@example.com",
        "address": "QUEVEDO",
        "queued_at": "2026-09-19T15:00:00",
    },
    "127.0.0.1",
)
assert lan_new["demographics_received"] is True, lan_new
new_pid = lan_new["clinical_patient_id"]
assert new_pid
...
assert created["name"] == "GUTIERREZ TROYA REINALDO PORFIRIO"
assert created["national_id_search"] == "1717171717"
assert created["birth_date"] == "1985-06-14"
assert created["phone"] == "0991234567"
assert created["email"] == "paciente@example.com"
assert created["address"] == "QUEVEDO"
qrow = check.execute(
    "SELECT id,clinical_patient_id FROM waiting_queue WHERE reception_event_id='event-new-patient'"
).fetchone()
assert qrow and qrow["clinical_patient_id"] == new_pid
queue_id = qrow["id"]
check.close()

# First click on ATENDER must go to Editar/Confirmar ficha, not to consultation.
response = app.attend_from_queue_v132(queue_id)
assert response.status_code == 303
assert f"/paciente/{new_pid}/editar?queue_id={queue_id}" in response.headers["location"]
assert "/nueva" not in response.headers["location"]

edit_page = app.edit_patient_page_v132(new_pid, queue_id=queue_id, nuevo="1", turno="")
edit_html = edit_page.body.decode("utf-8")
assert "Confirmar ficha" in edit_html
assert "Guardar y continuar" in edit_html
assert "1985-06-14" in edit_html
assert "0991234567" in edit_html
assert "paciente@example.com" in edit_html
assert "QUEVEDO" in edit_html

class FakeEditRequest:
    async def body(self):
        return urlencode({
            "queue_id": queue_id,
            "name": "GUTIERREZ TROYA REINALDO PORFIRIO",
            "birth_date": "1985-06-14",
            "sex": "",
            "civil_status": "",
...
saved_confirmation = asyncio.run(app.edit_patient_save_v132(new_pid, FakeEditRequest()))
assert saved_confirmation.status_code == 303
assert "confirmado=1" in saved_confirmation.headers["location"]

check = sqlite3.connect(ROOT / "data" / "historia_clinica.db")
check.row_factory = sqlite3.Row
confirmed_link = check.execute(
    "SELECT matched_by FROM patient_links WHERE reception_patient_id='rp-new-777'"
).fetchone()
check.close()
assert confirmed_link and confirmed_link["matched_by"] == "doctor_confirmed_new"

# After confirmation, the queue takes the doctor to the ficha with an ATENDER action.
response2 = app.attend_from_queue_v132(queue_id)
assert response2.status_code == 303
assert f"/paciente/{new_pid}?" in response2.headers["location"]
assert "confirmado=1" in response2.headers["location"]

patient_html = app.patient(new_pid).body.decode("utf-8")
assert "GUTIERREZ TROYA REINALDO PORFIRIO" in patient_html

# The guarded consultation route now allows the confirmed queue.
consult = app.new_consultation_v132(new_pid, encounter_id="", queue_id=queue_id)
assert getattr(consult, "status_code", 200) == 200
consult_html = consult.body.decode("utf-8")
assert "PACIENTE NUEVO" in consult_html

# Re-sending the same reception patient must reuse the deterministic ficha.
lan_again = service.accept_handoff(
    {
        "event_id": "event-new-patient-2",
        "reception_patient_id": "rp-new-777",
        "display_name": "GUTIERREZ TROYA REINALDO PORFIRIO",
        "identification": "1717171717",
        "attention_type": "Nuevo",
        "birth_date": "1985-06-14",
        "phone": "0991234567",
        "queued_at": "2026-09-19T15:30:00",
    },
    "127.0.0.1",
)
assert lan_again["clinical_patient_id"] == new_pid
check = sqlite3.connect(ROOT / "data" / "historia_clinica.db")
again = check.execute(
...
print("APP_IMPORT_OK", app.APP_VERSION, "UPPERCASE_OK", "LAN_CANCEL_SAFE_OK", "NEW_PATIENT_CONFIRM_FIRST_OK", "DEMOGRAPHICS_OK")
